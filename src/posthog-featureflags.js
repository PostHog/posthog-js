...
PostHogFeatureFlags.prototype.getFlags = function() {
    if(this._posthog.persistence.props['$override_feature_flags']) {
        if(!this._override_warning) console.warn('[PostHog] Overriding feature flags! Feature flags from server were: ' + JSON.stringify(this._posthog.persistence.props['$active_feature_flags']))
