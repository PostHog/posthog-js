...
export const isSessionReplayEnabled = (loader: SessionReplayLoader) => {
...
export class SessionReplayLoader {
    ...
    constructor(
        ...
        readonly isEnabled: (srl: SessionReplayLoader) => boolean
    ) {
        this.startIfEnabled()
    }
    ...
    private loadScript(cb: () => void): void {
        ...
        assignableWindow.__PosthogExtensions__?.loadExternalDependency?.(this.instance, 'session-replay', (err) => {
            if (err) {
