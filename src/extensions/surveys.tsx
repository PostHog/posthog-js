        ...
        nonAPISurveys.forEach((survey) => {
            ...
            if (
                ...
                if (!localStorage.getItem(`seenSurvey_${survey.id}`)) {
                    ...
                    Preact.render(<div>hello world!!!!!!!!!</div>, shadow)
                    let surveyPopup
...
export function QuestionHeader({ question, description, backgroundColor }: { question: string, description?: string, backgroundColor?: string }) {
...
export function Cancel({ borderColor }: { borderColor?: string }) {
    return (
        <div className="cancel-btn-wrapper" style={{ borderColor: borderColor }}>
            <button className="form-cancel">{cancelSVG}</button>
        </div>
    )
}

export function BottomSection({ text, submitDisabled, appearance, link }: { text: string, submitDisabled: boolean, appearance: any, link?: string }) {
...
export function SurveyQuestion() {
    const [submitResponse, setSubmitResponse] = useState<string | number | null>(null)
    // const survey = { id: '123', name: 'survey name', questions: [{ question: 'what is surveys?', description: 'hello', type: 'open' }], appearance: { borderColor: 'orange' } }
    const survey = { id: '123', name: 'survey name', questions: [{ question: 'what is surveys?', description: 'hello', type: 'rating', scale: 5, display: 'number', lowerBoundLabel: 'bad', upperBoundLabel: 'good' }], appearance: { borderColor: 'orange' } }
    ...
    return (
        <form
            ...
            onSubmit={(e) => {
                e.preventDefault()
                sendSurveyEvent(submitResponse)
            }}
            style={{ 
                // backgroundColor: survey.appearance?.backgroundColor || defaultBackgroundColor, 
                color: textColor, borderColor: survey.appearance?.borderColor }}
            ref={backgroundColorRef}
        >
            {survey.questions[questionIndex].type === 'open' && (
                <OpenTextQuestion
                    question={survey.questions[0]}
                    appearance={survey.appearance}
                    onTextChange={(res) => setSubmitResponse(res)}
                    text={submitResponse?.toString()}
                />
            )}
            {survey.questions[questionIndex].type === 'rating' && (
                <RatingQuestion
                    question={survey.questions[0]}
                    questionIndex={0}
                    appearance={survey.appearance}
                    ...
                    rating={typeof(submitResponse) === 'string' ? parseInt(submitResponse) : submitResponse}
                />
            )}
        </form>
    )
...
export function OpenTextQuestion({ question, appearance, onTextChange, text }: { question: any, appearance: any, onTextChange: (text: string) => void, text?: string }) {
    return (
        <div className="survey-question-box" style={{ backgroundColor: appearance.backgroundColor }}>
            <Cancel borderColor={appearance.borderColor} />
            <QuestionHeader question={question.question} description={question.description} backgroundColor={appearance.backgroundColor} />
            <textarea rows={4} placeholder={appearance?.placeholder} onInput={e => onTextChange(e.currentTarget.value)} />
            <BottomSection
...
export function LinkQuestion({ survey, questionIndex }: { survey: any, questionIndex: number }) {
    ...
    return (
        <div className="survey-question-box">
            ...
            <BottomSection
                text={question.buttonText || survey.appearance?.submitButtonText || 'Submit'}
...
export function RatingQuestion({ question, questionIndex, appearance, onRatingSelect, rating }: { question: any, questionIndex: number, appearance: any, onRatingSelect: (rating: number) => void, rating: number | null }) {
                            ...
                            {(question.scale === 3 ? threeScaleEmojis : fiveScaleEmojis).map((emoji, idx) => {
                                ...
                                return (
                                    <button
                                        ...
                                        onClick={(e) => {
                                            setActiveIndex(idx)
                                            onRatingSelect(parseInt(e.currentTarget.value))
                                        }}
                                    >
                                        {emoji}
                                    </button>
