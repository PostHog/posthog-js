...
describe('heatmaps', () => {
    ...
    let heatmapsAutocapture: HeatmapsAutocapture
    ...
    describe('lazily loaded heatmaps autocapture', () => {
        beforeEach(() => {
            posthog.config.capture_heatmaps = true
            heatmapsAutocapture = new HeatmapsAutocapture(posthog)
            // make sure we start fresh
            heatmapsAutocapture!.startIfEnabled()
            expect(heatmapsAutocapture!.getAndClearBuffer()).toBeUndefined()

            posthog.register({ $current_test_name: expect.getState().currentTestName })
        })

        it('should send generated heatmap data', async () => {
            heatmapsAutocapture?.['_onClick']?.(createMockMouseEvent())

            jest.advanceTimersByTime(heatmapsAutocapture!.flushIntervalMilliseconds + 1)
        ...
        it('requires interval to pass before sending data', async () => {
            ...
            expect(heatmapsAutocapture!.getAndClearBuffer()).toBeDefined()
        })
        ...
        it('should clear the buffer after each call', async () => {
            ...
            expect(heatmapsAutocapture!['buffer']).toEqual(undefined)
        ...
        it('should ignore clicks if they come from the toolbar', async () => {
            ...
            expect(heatmapsAutocapture?.getAndClearBuffer()).not.toEqual(undefined)
        ...
        describe('isEnabled()', () => {
            ...
            it.each([
                ...
                (deprecatedclientSideOptIn, clientSideOptIn, serverSideOptIn, expected) => {
                    ...
                    heatmapsAutocapture!.afterDecideResponse({
                        heatmaps: serverSideOptIn,
                    } as DecideResponse)
                    expect(heatmapsAutocapture!.isEnabled).toBe(expected)
                }
            )
        })
    })
