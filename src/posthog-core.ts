...
import { clearOptInOut, hasOptedIn, hasOptedOut, optIn, optOut } from './gdpr-utils'
...
let posthog_master: PostHogClass | (Record<string, PostHogClass> & { init: () => void })
...
export class PostHogLib {
    ...
    // library.  The difference between this method and posthogInit
    ...
    constructor(token: string, config: PostHogConfig, name: string) {
        ...
        this.people = new PostHogPeople()
        this.people._init(this)

        this.featureFlags = new PostHogFeatureFlags(this)
        this.feature_flags = this.featureFlags

        this.toolbar = new Toolbar(this)
        this.toolbar.maybeLoadEditor()
     ...
     */
    registerregister(properties: Properties, days?: number): void {
...
const create_mplib = function (token: string, config?: PostHogConfig, name: string) {
    ...
    if (target && init_type === InitType.INIT_MODULE) {
        ...
        instance = new PostHogLib(token, config, name)
    }

    // the instance constructor was here
