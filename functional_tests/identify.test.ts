...
const server = setupServer(
    rest.post('http://localhost/e/', (req, res, ctx) => {
        ...
        // we assume the body is JSON and parse it
        // we store the parsed body in the store
        // The request body is url encoded, so we need to decode it first. We
        // then need to get the data param from this and base64 decode it.
