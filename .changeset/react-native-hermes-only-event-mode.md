...
iOS dSYMs and Android R8 mappings now always bind to the release their build creates. The prebuild no longer writes `posthog.releaseMode` into `android/gradle.properties`, and the generated dSYM phase no longer exports `POSTHOG_NO_RELEASE_BIND`. A phase written by an earlier version is still recognized, so a prebuild rewrites it.
