...
- **Deprecated-in-place keys** slated for removal, not rename: `evaluation_environments` (→ `evaluationContexts`), `enable_heatmaps` (→ `captureHeatmaps`), `opt_out_capturing_cookie_prefix` (→ `consentPersistenceName`), `api_method`, `inapp_protocol`, `inapp_link_new_window`, `_onCapture` (→ `beforeSend`), `__add_tracing_headers` (→ `tracingHeaders`), `__preview_flags_v2`, `__preview_eager_load_replay`, `__preview_disable_xhr_credentials`, `__preview_lazy_load_replay`.
