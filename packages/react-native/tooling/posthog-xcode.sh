...
#     build settings supply matching coordinates below, so nothing has to be injected into the app.
#   symbol-set stamps the release onto the uploaded source maps instead, and an exception inherits
#     the release of the maps its frames resolved against.
