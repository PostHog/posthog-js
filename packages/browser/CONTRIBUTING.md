...
Modern Rolldown bundles use its built-in Oxc transformer with an ES2015 syntax ceiling and the existing minimum browser versions. Babel remains for `array.full.es5.js` (Oxc cannot emit ES5) and the three slim/extension entries: the pinned Oxc pipeline retains unused async/compression code in the extension bundle and fails the artifact gates. The required ABI source-map names now survive the direct Oxc experiment; see the [reproduction and measurements](scripts/investigate-oxc-bundles.md). TestCafe also still uses Babel. Terser remains the minifier for every runtime bundle.
