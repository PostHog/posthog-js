...
test.describe('surveys - renderSurveyPopup on demand', () => {
    test('renderSurveyPopup shows a survey popup when called with valid survey ID', async ({ page, context }) => {
        ...
        // Call renderSurveyPopup programmatically
        await page.evaluate(() => {
            // @ts-expect-error - posthog is added to window in test setup
            window.posthog.onSurveysLoaded(() => {
                // @ts-expect-error - posthog is added to window in test setup
                window.posthog.renderSurveyPopup('test-survey-123')
            })
        })
    ...
    test('renderSurveyPopup ignores survey popup delay when rendering on demand', async ({ page, context }) => {
        const surveysAPICall = page.route('**/surveys/**', async (route) => {
            await route.fulfill({
                json: {
        ...
        await page.evaluate(() => {
            ...
            window.posthog.onSurveysLoaded(() => {
                // @ts-expect-error - posthog is added to window in test setup
                window.posthog.renderSurveyPopup('test-survey-delay')
            })
        })
    ...
    test('renderSurveyPopup handles invalid survey ID gracefully', async ({ page, context }) => {
        const surveysAPICall = page.route('**/surveys/**', async (route) => {
            await route.fulfill({
                json: {
                    surveys: [testSurvey],
        ...
        // Call renderSurveyPopup with invalid survey ID
        await page.evaluate(() => {
            // @ts-expect-error - posthog is added to window in test setup
            window.posthog.renderSurveyPopup('non-existent-survey')
        })
    ...
    test('renderSurveyPopup can be called multiple times for the same survey', async ({ page, context }) => {
        const surveysAPICall = page.route('**/surveys/**', async (route) => {
            await route.fulfill({
                json: {
                    surveys: [testSurvey],
        ...
        // Call renderSurveyPopup first time
        await page.evaluate(() => {
            // @ts-expect-error - posthog is added to window in test setup
            window.posthog.renderSurveyPopup('test-survey-123')
        })
    ...
    test('renderSurveyPopup fails gracefully when surveys are not initialized', async ({ page, context }) => {
        ...
        // Call renderSurveyPopup when surveys are not initialized
        await page.evaluate(() => {
            // @ts-expect-error - posthog is added to window in test setup
            window.posthog.renderSurveyPopup('test-survey-123')
        })
    ...
    test('renderSurveyPopup can render multiple different surveys', async ({ page, context }) => {
        const survey1 = {
            ...testSurvey,
            id: 'survey-1',
            name: 'First Survey',
        ...
        // Call renderSurveyPopup for first survey
        await page.evaluate(() => {
            // @ts-expect-error - posthog is added to window in test setup
            window.posthog.renderSurveyPopup('survey-1')
        })
        ...
        // Call renderSurveyPopup for second survey
        await page.evaluate(() => {
            // @ts-expect-error - posthog is added to window in test setup
            window.posthog.renderSurveyPopup('survey-2')
        })
