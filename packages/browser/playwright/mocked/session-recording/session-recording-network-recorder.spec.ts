...
test.beforeEach(async ({ context }) => {
    await context.route('https://example.com/', async (route) => {
        ...
        expect(route.request().postData()).toMatch(/^i am the (fetch|xhr) body$/)
        await route.fulfill({
            status: 200,
            json: {
        ...
        test.describe(`Session recording - network recorder - streamNetworkBody ${
            ...
            test.beforeEach(async ({ page, context }) => {
                    ...
                    await pg.evaluate(() => {
                        ;(window as any).wrapFetchForTesting = (badlyBehaved: boolean) => {
                            ...
                            window.fetch = async function (
                                ...
                                ;((window as any).fetchWrapperModes ??= []).push(badlyBehaved)
                ...
                await page.evaluate((isBadlyBehaved) => {
                    ;(window as any).wrapFetchForTesting(isBadlyBehaved)
            ...
            ;['fetch', 'xhr'].forEach((networkType) => {
                test('it captures ' + networkType, async ({ page }) => {
                    ...
                    let capturedRequests: Record<string, any>[] = []
                    await expect
                        .poll(async () => {
                            const snapshots = (await page.capturedEvents()).filter((c) => c.event === '$snapshot')
                            capturedRequests = snapshots.flatMap((snapshot) =>
                                snapshot.properties['$snapshot_data'].flatMap((data) =>
                                    data.type === 6 ? (data.data.payload.requests ?? []) : []
                                )
                            )
                            return capturedRequests.some(
                                (request) => request.name === 'https://example.com/' && request.responseBody
                            )
                        })
                        .toBe(true)

                    if (networkType === 'fetch') {
                        const modes = await page.evaluate(() => (window as any).fetchWrapperModes)
                        expect(modes.length).toBeGreaterThan(0)
                        expect(modes.every((mode) => mode === isBadlyBehavedWrapper)).toBe(true)
                    }
                    ...
                    expect(hasEntry(/https:\/\/localhost:\d+\/array\/test%20token\/config$/, 'fetch')).toBe(true)
                    expect(hasEntry(/https:\/\/localhost:\d+\/static\/(lazy-)?recorder.js(?:\?|$)/, 'script')).toBe(
                        true
                    )
