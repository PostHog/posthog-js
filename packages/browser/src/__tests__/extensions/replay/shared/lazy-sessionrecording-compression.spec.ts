...
import type { ReplayRecorderClient } from '@posthog/browser-common/replay/host'
...
async function setupLazyLoadedSessionRecording({ gzipSupported, gzipCompress }: SetupOptions) {
    ...
    const [
        ...
        { TestClient },
        { createDisposable },
        { SESSION_RECORDING_REMOTE_CONFIG, SESSION_RECORDING_IS_SAMPLED, SESSION_RECORDING_FLUSHED_SIZE },
    ] = await Promise.all([
        import('../../../../extensions/replay/external/lazy-loaded-session-recorder'),
        import('../../../../extensions/replay/globals'),
        import('../../../../../../browser-common/tests/helpers/test-client'),
        import('@posthog/browser-common'),
        import('../../../../extensions/replay/constants'),
    ])

    const base = new TestClient({
        session: { sessionId: 'sessionId', windowId: 'windowId', sessionStartTimestamp: Date.now() },
    })
    base.kv.set({
    ...
    options.apiHost = 'https://test.com'
    options.capturePageview = false
    options.consoleLogRecordingEnabled = false
    options.recording = { maskAllInputs: false, compress_events: true }
    const lazyLoadedSessionRecording = new LazyLoadedSessionRecording(client, () => options)
...
describe('LazyLoadedSessionRecording compression paths', () => {
    ...
    it.each([
        ...
        const { emit, captureSnapshot, lazyLoadedSessionRecording } = await setupLazyLoadedSessionRecording({
    ...
    it('flushes in-flight async compression before stop teardown', async () => {
        ...
        const { emit, captureSnapshot, lazyLoadedSessionRecording, stopRrweb } = await setupLazyLoadedSessionRecording({
            gzipSupported: true,
            gzipCompress,
        })
    ...
    it('drops only the event that is too large to stringify, so the rest of the queue still ships on unload', async () => {
        ...
        try {
            ...
            const captured = captureSnapshot.mock.calls.map(([, properties]: any[]) => properties.$snapshot_data)
    ...
    it('ships a full snapshot under the new session id when the recorder restarts while idle', async () => {
        const { emit, captureSnapshot, base, lazyLoadedSessionRecording } = await setupLazyLoadedSessionRecording({
            gzipSupported: true,
        })
        ...
        base.session = { sessionId: 'rotated-session-id', windowId: 'windowId', sessionStartTimestamp: 123 }
    ...
    it('requests a full snapshot when an incremental ships for a rotated session without one', async () => {
        const { emit, base, lazyLoadedSessionRecording, record } = await setupLazyLoadedSessionRecording({
            gzipSupported: true,
        })
        const takeFullSnapshot = record.takeFullSnapshot
