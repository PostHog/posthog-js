...
describe('SurveyManager', () => {
    ...
    describe('renderSurvey with URL prefill', () => {
        ...
        it('advances past the prefilled question when localStorage is unavailable', async () => {
            ...
            const storageSpies = (['getItem', 'setItem', 'removeItem'] as const).map((method) =>
                vi.spyOn(Storage.prototype, method).mockImplementation(() => {
                    throw opaqueOriginError
                })
            )
            ...
            try {
                ...
                const survey: Survey = {
                    id: 'prefill-no-storage-survey',
                    name: 'Prefill No Storage Survey',
                    ...
                    questions: [
                        {
                            id: 'q1',
                            type: SurveyQuestionType.Rating,
                            question: 'Rate the report',
                            scale: 2,
                            display: 'emoji',
                            skipSubmitButton: true,
                        },
                        { id: 'q2', type: SurveyQuestionType.Open, question: 'Tell us more', optional: true },
                    ],
                    appearance: {},
                    conditions: null,
                    start_date: '2021-01-01T00:00:00.000Z',
