...
describe('RageClick()', () => {
    ...
    describe('when disabled or invalid config', () => {
        it('handles false as input gracefully', () => {
            ...
            const enabled = new RageClick(true)
            expect(enabled.isRageClick(0, 0, 0)).toBe(false)
            expect(enabled.isRageClick(0, 0, 100)).toBe(false)
            expect(enabled.isRageClick(0, 0, 200)).toBe(true)
        })
    })
})
