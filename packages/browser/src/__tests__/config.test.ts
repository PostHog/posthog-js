...
describe('config', () => {
    ...
    describe('compatibilityDate', () => {
        ...
        it('keeps capture_copied_text opt-in with the 2026-08-30 defaults', () => {
            ...
            const autocapture = new BrowserAutocapture(posthog)
            expect(autocapture['_refreshConfig']().capture_copied_text).toBeFalsy()
            posthog.set_config({ autocapture: { capture_copied_text: true } })
            expect(autocapture['_refreshConfig']().capture_copied_text).toBe(true)
        })
