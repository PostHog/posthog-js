...
// and the types the declarations use (`ReactElement<any, any>`) are available from the same release.
...
describe('React entrypoint dependency metadata', () => {
    ...
    it('declares React types as an optional peer dependency', () => {
        expect(packageInfo.peerDependencies['@types/react']).toBe(REACT_MINIMUM_VERSION)
