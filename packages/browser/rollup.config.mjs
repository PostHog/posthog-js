...
const plugins = (fileName) => {
    const [es5, noExternal] = [fileName.includes('es5'), fileName.includes('no-external')]
    const dirName = `./debug/${fileName}`
    if (!fs.existsSync(dirName)) {
        fs.mkdirSync(dirName, { recursive: true })
    }
    const nameCachePath = `${dirName}/terser-mangled-names.json`
    const bundleStatsPath = `${dirName}/bundle-stats.html`
    return [
        json(),
        ...
        }),
        {
            ...
            writeBundle() {
                if (!WRITE_MANGLED_PROPERTIES || !nameCache.props.props) {
                    return
                }
        ...
        visualizer({ filename: bundleStatsPath, gzipSize: true }),
    ]
}

function distTarget(file) {
    ...
    const pluginsForThisFile = plugins(fileName)
...
export default (cliArgs) => {
    const entrypoint = cliArgs.entrypoint
    if (!entrypoint) {
        throw new Error('No entrypoint specified')
    }
    const file = path.basename(entrypoint)
    delete cliArgs.entrypoint
    const config = [distTarget(file)]
    if (file.endsWith('.es.ts')) {
        config.push(typeTarget(file))
    }
    return config
}
