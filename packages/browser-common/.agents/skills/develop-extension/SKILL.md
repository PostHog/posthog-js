...
| record an event               | `client.capture(event, properties?, options?)`             |
...
- **Initialize KV, then use it synchronously.** `client.kv.initialize()` may be awaitable while a host hydrates its
  ...
  durable flushing. Identity, session, `canCapture`, `projectToken`, and capture admission are also synchronous, while
  requests are awaitable and remote-config outcomes are delivered through `onRemoteConfig`.
