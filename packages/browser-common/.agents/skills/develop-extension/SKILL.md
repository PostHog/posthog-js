...
prefix. `onRemoteConfig` replays the latest available outcome when host capabilities permit; consent-aware hosts can
defer requests and replay while retaining the subscription. Narrow on `result.ok` before reading `result.config` and
define safe behavior for `{ ok: false }`.
