...
import type { EventName, Properties } from '@posthog/types'
...
import { convertToURL } from './utils/convert-to-url'
...
export function autocapturePropertiesForElement(
    ...
    if (href) {
        ...
        const locationHost = getWindow()?.location?.host
...
export class Autocapture implements Extension {
    ...
    private _listenerWindow: Window | undefined
    private _listenerDocument: Document | undefined
    ...
    private _domEventHandler: EventListener | undefined
    private _copiedTextHandler: EventListener | undefined
    ...
    private _pointerDown:
        ...
        | undefined
    private _pointerExpiry: ReturnType<typeof setTimeout> | undefined
    ...
    private _recoverClickTarget(e: PointerEvent): Element | null | undefined {
        ...
        if (
            ...
            const ignorelist = this._refreshConfig().css_selector_ignorelist ?? DEFAULT_AUTOCAPTURE_IGNORE_LIST
            const excludedRoot = [target, getDocument()?.documentElement, getDocument()?.body].some((root) => {
    ...
    private _compileUrlPatterns(): Readonly<AutocaptureConfig> {
        this._refreshConfig()
        // Keep explicit undefined properties while compiling the mutable legacy snapshot.
        this._config.url_allowlist = this._config.url_allowlist?.map((url) => new RegExp(url)) as NonNullable<
            AutocaptureConfig['url_allowlist']
        >
        this._config.url_ignorelist = this._config.url_ignorelist?.map((url) => new RegExp(url)) as NonNullable<
            AutocaptureConfig['url_ignorelist']
        >
        return this._config
    }
    ...
    _addDomEventHandlers(): void {
        ...
        this._listenerDocument = document
    ...
    private _removeDomEventHandlers(): void {
        const window = this._listenerWindow
        const document = this._listenerDocument
        this._clearPointer()
        ...
        this._listenerWindow = undefined
        this._listenerDocument = undefined
    ...
    public getElementSelectors(element: Element | null, additionalElement?: Element): string[] | null {
        ...
        this._elementSelectors?.forEach((selector) => {
            const matchedElements = getDocument()?.querySelectorAll(selector)
    ...
    private _captureEvent(e: Event, eventName: EventName = '$autocapture', targetOverride?: Element): boolean | void {
        ...
        if (
            ...
            shouldCaptureDomEvent(
                target,
                e,
                // Preserve the explicit undefined clipboard allowlist in the legacy config shape.
                eventConfig as AutocaptureConfig,
    ...
    isBrowserSupported(): boolean {
        return isFunction(getDocument()?.querySelectorAll)
    }
}
