...
export interface ConsoleLogsSink {
...
export interface ConsoleLogsHost {
    console: Console
    hostname: string
    getCapturingLogs(): ConsoleLogsSink | undefined
}
...
export const initializeLogs = (host: ConsoleLogsHost): (() => void) => {
    const console = host.console
    let currentHost: ConsoleLogsHost | undefined = host
    ...
    const attributes: Record<string, string> = { host: host.hostname }
    ...
    const dispose = () => {
        ...
        for (const restore of restoreConsoleMethods) {
            try {
                restore()
            } catch {
                /* Other console layers still need cleanup. */
            }
        }
    }
    try {
        for (const level of Object.keys(LEVEL_MAP) as BufferedConsoleLevel[]) {
                ...
                (...args: any[]) => {
                    ...
                    try {
                        ...
                        if (args.length > 0 && !isCapturingLog && activeHost) {
                            const logs = activeHost.getCapturingLogs()
                            if (logs) {
                                isCapturingLog = true
                                acquiredGuard = true
                                const {
            ...
            const consoleBeforePatch = console[level]
            restoreConsoleMethods.push(
                patch(console, level, (next: any) => {
...
export const replayConsoleBuffer = (host: ConsoleLogsHost, entries: BufferedConsoleEntry[]) => {
    const logs = host.getCapturingLogs()
    if (!logs) {
        return
    }
