import type { SurveyActionHost, SurveyCapturedEvent as CaptureResult } from './survey-event-host'
import type { ActionStepType, PropertyFilters, SurveyActionType, SurveyElement } from './types/surveys'
...
export class ActionMatcher {
    ...
    constructor(private readonly _host?: SurveyActionHost) {}
    ...
    init() {
        if (!isUndefined(this._host?.subscribeCapture)) {
            ...
            this._captureHookUnsubscribe = this._host?.subscribeCapture(matchEventToAction)
        }
    }
    ...
    register(actions: SurveyActionType[]): void {
        ...
        if (this._host?.setElementSelectors) {
            ...
            this._host.setElementSelectors(selectorsToWatch)
        }
    }
