...
export function entries<T = any>(obj: Record<string, T>): [string, T][] {
    ...
    const resArray = new Array<[string, T]>(i) // preallocate the Array
...
export const safewrapClass = function (klass: Function, functions: string[]): void {
    for (let i = 0; i < functions.length; i++) {
        klass.prototype[functions[i] as string] = safewrap(klass.prototype[functions[i] as string])
    }
}
...
export function isToolbarInstance(config: Pick<{ name: string }, 'name'>): boolean {
