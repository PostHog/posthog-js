...
export class UUID {
    ...
    toString(): string {
        ...
        for (let i = 0; i < this.bytes.length; i++) {
            const byte = this.bytes[i] as number
            text = text + (byte >>> 4).toString(16) + (byte & 0xf).toString(16)
            if (i === 3 || i === 5 || i === 7 || i === 9) {
                text += '-'
            }
        }
    ...
    compareTo(other: UUID): number {
        for (let i = 0; i < 16; i++) {
            const diff = (this.bytes[i] as number) - (other.bytes[i] as number)
            if (diff !== 0) {
...
if (!isUndefined(cryptoRef) && cryptoRef.getRandomValues) {
    getRandomValues = <T extends Uint8Array | Uint32Array>(buffer: T): T =>
        cryptoRef.getRandomValues(buffer as any) as T
}
