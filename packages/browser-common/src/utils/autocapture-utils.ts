import { each, entries, type Properties } from './general-utils'
...
export type DeadClickCaptureConfig =
    | boolean
    | {
          css_selector_ignorelist?: string[]
      }
...
export function shouldCaptureDeadClick(el: Element | null, _config: DeadClickCaptureConfig) {
...
export function shouldCaptureRageclick(el: Element | null, _config: RageClickConfig) {
    if (!window || cannotCheckForAutocapture(el)) {
        return false
    }
