...
export class PostHogSurveys implements Extension {
    ...
    public _surveyEventReceiver: SurveysEventReceiver | null = null
    private _surveyManager: SurveysManager | null = null
    ...
    reset(): void {
        // Clear fallback answers even when browser storage is unavailable.
    ...
    canRenderSurvey(surveyId: string | Survey): SurveyRenderReason {
        ...
        // Keep the callback/result shape identical when no disabled reason is present.
        // oxlint-disable-next-line typescript/consistent-type-assertions
        return { visible: eligibility.eligible, disabledReason: eligibility.reason } as SurveyRenderReason
    }
        ...
        return new Promise<SurveyRenderReason>((resolve) => {
            this.getSurveys((surveys) => {
                ...
                if (!survey) {
                    ...
                    // oxlint-disable-next-line typescript/consistent-type-assertions
                    resolve({ visible: eligibility.eligible, disabledReason: eligibility.reason } as SurveyRenderReason)
                }
            }, forceReload)
        })
    }
    ...
    renderSurvey(surveyId: string | Survey, selector: string, properties?: Properties) {
        ...
        const elem = (typeof document === 'undefined' ? undefined : document)?.querySelector(selector)
        if (!elem) {
