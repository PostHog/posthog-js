import type { SurveyActionType, SurveyEventWithFilters } from './types/surveys'
import { SurveyEventType } from './survey-constants'
import { ActionMatcher } from './survey-action-matcher'
import type { SurveyEventHost, SurveyCapturedEvent as CaptureResult } from './survey-event-host'
import { matchPropertyFilters } from '@posthog/core/surveys'
...
export abstract class EventReceiver<T extends EventTriggerable> {
    ...
    protected _actionMatcher: ActionMatcher | null | undefined
    protected readonly _host: SurveyEventHost
    /**
    ...
    constructor(host: SurveyEventHost) {
        this._host = host
    ...
    /** Start after subclass dependencies are initialized; hosts may publish synchronously. */
    protected _subscribeSession(): void {
        ...
        this._sessionIdUnsubscribe = this._host.subscribeSession?.((sessionId) => this._onSessionIdChanged(sessionId))
    }
    ...
    private _register(items: T[], replace: boolean): void {
        if (isUndefined(this._host.subscribeCapture)) {
            return
        }
    ...
    private _setupActionBasedItems(items: T[], replace: boolean): void {
        ...
        if (!this._actionMatcher) {
            this._actionMatcher = new ActionMatcher(this._host)
    ...
    private _setupEventBasedItems(items: T[], replace: boolean): void {
        ...
        this._captureHookUnsubscribe ??= this._host.subscribeCapture?.(matchEventToItem)
    }
    ...
    private _activateItems(itemIds: string[]): void {
        ...
        const canPersist = !!this._host.getSessionId()
    ...
    private _getRawActivationTimestamps(): Record<string, number> {
        ...
        const stored = this._host.getProperty(key)
    ...
    private _getRawPersistedActivatedIds(): string[] {
        ...
        const existingActivatedItems = this._host.getProperty<string[]>(activatedKey)
    ...
    private _getPersistedActivatedIds(): string[] {
        ...
        const stampedSessionId = this._host.getProperty(this._getActivatedSessionKey())
        ...
        const currentSessionId = this._host.getSessionId()
