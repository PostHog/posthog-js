...
import { SURVEY_IN_PROGRESS_PREFIX } from '../src/utils/survey-utils'
...
describe('survey display logic', () => {
    ...
    test('callSurveysAndEvaluateDisplayLogic runs on interval irrespective of url change', () => {
        ...
        try {
            expect(host.getSurveys).toBeCalledTimes(1)
            expect(setInterval).toHaveBeenLastCalledWith(expect.any(Function), 1000)

            vi.advanceTimersByTime(1000)
            expect(host.getSurveys).toBeCalledTimes(2)
            expect(setInterval).toHaveBeenLastCalledWith(expect.any(Function), 1000)
        } finally {
...
describe('usePopupVisibility', () => {
    ...
    test('should set isPopupVisible to true immediately if delay is 0', () => {
        const { result } = renderHook(() => usePopupVisibility(mockSurvey, host, 0, false, removeSurvey, false))
        expect(result.current.isPopupVisible).toBe(true)
    })

    test('should set isPopupVisible to true after delay', () => {
        vi.useFakeTimers()
        const { result } = renderHook(() => usePopupVisibility(mockSurvey, host, 1000, false, removeSurvey, false))
    ...
    test('should clean up event listeners and timers on unmount', () => {
        vi.useFakeTimers()
        const { unmount } = renderHook(() => usePopupVisibility(mockSurvey, host, 1000, false, removeSurvey, false))
    ...
    test('should set isPopupVisible to true if isPreviewMode is true', () => {
        const { result } = renderHook(() => usePopupVisibility(mockSurvey, host, 1000, true, removeSurvey, false))
        expect(result.current.isPopupVisible).toBe(true)
    })

    test('should set isPopupVisible to true after a delay of 500 milliseconds', () => {
        vi.useFakeTimers()
        const { result } = renderHook(() => usePopupVisibility(mockSurvey, host, 500, false, removeSurvey, false))
        expect(result.current.isPopupVisible).toBe(false)
        act(() => {
    ...
    test('should not throw an error if posthog is undefined', () => {
        const { result } = renderHook(() => usePopupVisibility(mockSurvey, undefined, 0, false, removeSurvey, false))
        expect(result.current.isPopupVisible).toBe(true)
    })

    test('should clean up event listeners on unmount when delay is 0', () => {
        const { unmount } = renderHook(() => usePopupVisibility(mockSurvey, host, 0, false, removeSurvey, false))
        const removeEventListenerSpy = vi.spyOn(window, 'removeEventListener')
    ...
    test('should dispatch PHSurveyShown event when survey is shown', () => {
        ...
        renderHook(() => usePopupVisibility(mockSurvey, host, 0, false, removeSurvey, false))
    ...
    test('should handle multiple surveys with overlapping conditions', () => {
        ...
        const { result: result1 } = renderHook(() =>
            usePopupVisibility(mockSurvey, host, 0, false, removeSurvey, false)
        )
        const { result: result2 } = renderHook(() =>
            usePopupVisibility(mockSurvey2, host, 500, false, removeSurvey, false)
        )
...
describe('SurveyManager', () => {
    ...
    beforeEach(() => {
        ...
        host = createSurveysRuntimeHost({
            ...
            capture: vi.fn(),
            hasLoadedFlags: true,
            getFlag: vi.fn().mockImplementation((featureFlag) => flagsResponse.featureFlags[featureFlag]),
            isFlagEnabled: vi.fn().mockImplementation((featureFlag) => flagsResponse.featureFlags[featureFlag]),
            getSurveys: vi.fn().mockImplementation((callback) => callback(mockSurveys)),
        })

        surveyManager = createSurveyManager(host)
    })
    ...
    afterEach(() => {
        ...
        // window.dispatchEvent(new Event('languagechange')), hitting a stale host whose
    ...
    test('callSurveysAndEvaluateDisplayLogic should handle a single popover survey correctly', () => {
        ...
        expect(host.getSurveys).toHaveBeenCalled()
    ...
    it('should only display one popover survey if multiple popovers are eligible', () => {
        ...
        host.getSurveys = vi.fn((callback) => callback([mockSurveys[0], anotherPopover]))
    ...
    describe('reports when capturing is opted out', () => {
        ...
        it('removes a tab widget that is already on screen', () => {
            ...
            const originalGetSurveys = host.getSurveys
            host.getSurveys = vi.fn((callback: (surveys: Survey[]) => void) => callback([widgetSurvey]))
            ...
            try {
                ...
                host.getSurveys = originalGetSurveys
        ...
        it('does not open a tab clicked after opt-out but before the next display poll', () => {
            ...
            Object.defineProperty(host, 'canCapture', { get: isCapturing, configurable: true })
            const widgetSurvey: Survey = {
                ...mockSurveys[0],
                type: SurveyType.Widget,
    ...
    describe('warns when advanced_disable_feature_flags hides a survey', () => {
        ...
        it('names the survey-only escape hatch, once per instance', () => {
            host.featureFlagEvaluation = false
    ...
    test('manageWidgetSelectorListener should be called for selector widgets', () => {
        ...
        host.getSurveys = vi.fn((callback) => callback([mockSurvey]))
    ...
    it('does not tear down an open selector-widget survey when its trigger element unmounts (issue #2036)', async () => {
        ...
        host.getSurveys = vi.fn().mockImplementation((callback) => callback([survey]))
        document.body.innerHTML = '<div class="my-selector">Click Me</div>'
    ...
    test('callSurveysAndEvaluateDisplayLogic should not call surveys in focus', () => {
        host.getSurveys = vi.fn((callback) => callback(mockSurveys))

        surveyManager.getTestAPI().addSurveyToFocus({ id: 'survey1' })
        surveyManager.callSurveysAndEvaluateDisplayLogic()

        expect(host.getSurveys).toHaveBeenCalledTimes(1)
    ...
    test('surveyInFocus handling works correctly with in callSurveysAndEvaluateDisplayLogic', () => {
        ...
        expect(host.getSurveys).toHaveBeenCalledTimes(2)
        expect(surveyManager.getTestAPI().surveyInFocus).toBe(null)
    ...
    describe('renderSurvey with URL prefill', () => {
        ...
        it.each([
            ...
            jsdom.reconfigure({ url: new URL(search, window.location.href).href })
        ...
        it('retains the auto-advanced prefilled answer through a later manual submit', async () => {
            ...
            jsdom.reconfigure({ url: new URL('?q0=1', window.location.href).href })
            const surveyDiv = document.createElement('div')
            surveyManager.renderSurvey(survey, surveyDiv)
    ...
    describe('re-renders the actual popup on a real language change', () => {
        ...
        it('updates the rendered question text and keeps the typed answer when the language changes', () => {
            ...
            host.getCachedSurveys = vi.fn().mockReturnValue([langSurvey])

            surveyManager.handlePopoverSurvey(langSurvey)

            const { shadow } = retrieveSurveyShadow(langSurvey, host)
        ...
        it('keeps the shuffled question order stable across a language change', () => {
            ...
            for (let trial = 0; trial < 10; trial++) {
                ...
                setNavigatorLanguage('en')
                host.getCachedSurveys = vi.fn().mockReturnValue([shuffledSurvey])

                surveyManager.handlePopoverSurvey(shuffledSurvey)

                const { shadow } = retrieveSurveyShadow(shuffledSurvey, host)
    ...
    describe('checkFlags', () => {
        ...
        it('should return true when all feature flags are enabled', () => {
            ...
            vi.spyOn(host, 'isFlagEnabled').mockImplementation(() => true)
        ...
        it('should return false when any feature flag is disabled', () => {
            ...
            vi.spyOn(host, 'isFlagEnabled').mockImplementation((flag) => (flag === 'flag-1' ? true : false))
    ...
    describe('URL prefill auto-submit behavior', () => {
        ...
        it('should auto-submit prefilled responses when skipSubmitButton is true and enable_partial_responses is true', () => {
            ...
            jsdom.reconfigure({ url: new URL('?q0=8', window.location.href).href })
        ...
        it('should only include skipped responses in auto-submit, not all prefilled responses', () => {
            ...
            jsdom.reconfigure({ url: new URL('?q0=8&q1=5', window.location.href).href })
            ;(surveyManager as any)._handleUrlPrefill(survey)
...
describe('usePopupVisibility URL changes should hide surveys accordingly', () => {
    ...
    afterEach(() => {
        ...
        jsdom.reconfigure({ url: originalLocationHref })
    })
    ...
    it('should not hide survey when URL matches - exact match', () => {
        ...
        jsdom.reconfigure({ url: 'https://example.com/path1' })
        const { result } = renderHook(() =>
            usePopupVisibility(survey, posthog, 0, false, mockRemoveSurveyFromFocus, false)
        )
    ...
    it('should not hide survey when URL matches - contains', () => {
        ...
        jsdom.reconfigure({ url: 'https://example.com/path' })
    ...
    it('should handle browser back/forward navigation', () => {
        ...
        act(() => {
            jsdom.reconfigure({ url: 'https://example.com/other/page' })
    ...
    it('should handle hash-based navigation', () => {
        ...
        act(() => {
            jsdom.reconfigure({ url: 'https://example.com/other/page#/hash' })
    ...
    it('should not show delayed survey if URL no longer matches when delay expires', () => {
        ...
        jsdom.reconfigure({ url: 'https://example.com/initial-path' })
        ...
        const { result } = renderHook(() =>
            usePopupVisibility(survey, posthog, 2000, false, mockRemoveSurveyFromFocus, false)
        )
        ...
        act(() => {
            jsdom.reconfigure({ url: 'https://example.com/different-path' })
            window.dispatchEvent(new Event('popstate'))
        })
...
describe('useHideSurveyOnURLChange', () => {
    ...
    it('should not do anything if no URL conditions are set', () => {
        const survey = {
            ...BASE_SURVEY,
            conditions: { cancelEvents: null, events: null, actions: null },
        }
    ...
    it('should handle popstate events', () => {
        ...
        act(() => {
            jsdom.reconfigure({ url: 'https://different.com' })
            window.dispatchEvent(new Event('popstate'))
        })
    ...
    it('should handle hashchange events', () => {
        ...
        act(() => {
            jsdom.reconfigure({ url: 'https://different.com/#/some-hash' })
            window.dispatchEvent(new Event('hashchange'))
        })
