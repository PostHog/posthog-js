...
describe('PostHog Feature Flags v4', () => {
  ...
  describe('featureflags', () => {
    ...
    describe('when loaded', () => {
      ...
      describe('variant change tracking', () => {
        it('should track when flag value changes from boolean true to variant string', async () => {
          // Initial call returns true (boolean)
          const initialFlags = {
            flags: {
              'multivariate-flag': {
                key: 'multivariate-flag',
                enabled: true,
                variant: undefined, // undefined variant means boolean true
                reason: undefined,
                metadata: undefined,
              },
            },
          } as unknown as PostHogV2FlagsResponse
          posthog.setPersistedProperty(
            PostHogPersistedProperty.FeatureFlagDetails,
            normalizeFlagsResponse(initialFlags)
          )

          // First call should return true and send event
          expect(posthog.getFeatureFlag('multivariate-flag')).toEqual(true)
          await waitForPromises()
          const firstEventIndex = mocks.fetch.mock.calls.length - 1
          expect(parseBody(mocks.fetch.mock.calls[firstEventIndex])).toMatchObject({
            batch: [
              {
                event: '$feature_flag_called',
                properties: {
          ...
          // Update flag to return variant string
          const updatedFlags = {
            flags: {
              'multivariate-flag': {
                key: 'multivariate-flag',
                enabled: true,
                variant: 'control', // now returns variant string
                reason: undefined,
                metadata: undefined,
              },
            },
          ...
          posthog.setPersistedProperty(
            PostHogPersistedProperty.FeatureFlagDetails,
            normalizeFlagsResponse(updatedFlags)
          )

          // Second call should return variant and send new event
          expect(posthog.getFeatureFlag('multivariate-flag')).toEqual('control')
          await waitForPromises()
          const secondEventIndex = mocks.fetch.mock.calls.length - 1
          expect(secondEventIndex).toBeGreaterThan(firstEventIndex)
          expect(parseBody(mocks.fetch.mock.calls[secondEventIndex])).toMatchObject({
            batch: [
              {
                event: '$feature_flag_called',
                properties: {
        ...
        it('should track when variant value changes between different strings', async () => {
          ...
          // First call
          expect(posthog.getFeatureFlag('ab-test-flag')).toEqual('variant-a')
          await waitForPromises()
          const firstCallCount = mocks.fetch.mock.calls.length
          ...
          // Second call should send new event for different variant
          expect(posthog.getFeatureFlag('ab-test-flag')).toEqual('variant-b')
          await waitForPromises()
          expect(mocks.fetch.mock.calls.length).toBeGreaterThan(firstCallCount)
        })

        it('should not send duplicate events when variant stays the same', async () => {
          // Set up flag with variant
          const flags = {
            flags: {
              'stable-flag': {
                key: 'stable-flag',
          ...
          posthog.setPersistedProperty(PostHogPersistedProperty.FeatureFlagDetails, normalizeFlagsResponse(flags))

          // First call
          expect(posthog.getFeatureFlag('stable-flag')).toEqual('stable-variant')
          await waitForPromises()
          const firstCallCount = mocks.fetch.mock.calls.length

          // Second call with same value should not send new event
          expect(posthog.getFeatureFlag('stable-flag')).toEqual('stable-variant')
          await waitForPromises()
          expect(mocks.fetch.mock.calls.length).toEqual(firstCallCount)
        })

        it('should track when flag changes from variant back to boolean true', async () => {
          // Start with variant
          const initialFlags = {
            flags: {
              'toggle-flag': {
          ...
          // First call with variant
          expect(posthog.getFeatureFlag('toggle-flag')).toEqual('test-variant')
          await waitForPromises()
          const firstCallCount = mocks.fetch.mock.calls.length

          // Change to boolean true (no variant)
          const updatedFlags = {
            flags: {
              'toggle-flag': {
                key: 'toggle-flag',
          ...
          // Second call should return true and send new event
          expect(posthog.getFeatureFlag('toggle-flag')).toEqual(true)
          await waitForPromises()
          expect(mocks.fetch.mock.calls.length).toBeGreaterThan(firstCallCount)
        })
      })
