...
describe('push', () => {
    it('creates a workflow PostHog does not have, sending the key and source fields', async () => {
    ...
    it('writes nothing on a second push that changes nothing, and says the commit was not recorded', async () => {
        const standIn = await startStandIn()
        onTestFinished(() => standIn.close())
        const workspace = makeWorkspace({ 'flows/onboarding.ts': workflowFile() })
        const env = {
    ...
    it('updates the workflow it created, and leaves the key out of the update', async () => {
        const standIn = await startStandIn()
        onTestFinished(() => standIn.close())
        const workspace = makeWorkspace({ 'flows/onboarding.ts': workflowFile() })
    ...
    it('updates when only a step description changes', async () => {
        const standIn = await startStandIn()
        onTestFinished(() => standIn.close())
        const workspace = makeWorkspace({ 'flows/onboarding.ts': workflowFile() })
        await push(workspace, standIn)
    ...
    it('does not send or compare status when the file sets none', async () => {
        const standIn = await startStandIn()
        onTestFinished(() => standIn.close())
    ...
    it('sends and compares status when the file sets it', async () => {
        const standIn = await startStandIn()
        onTestFinished(() => standIn.close())
    ...
    it('claims code ownership on the create and on the update', async () => {
        const standIn = await startStandIn()
        onTestFinished(() => standIn.close())
        const workspace = makeWorkspace({ 'flows/onboarding.ts': workflowFile() })
        await push(workspace, standIn)
    ...
    it('refuses a missing secret even when nothing else changed', async () => {
        const standIn = await startStandIn()
        onTestFinished(() => standIn.close())
    ...
    it('writes a secret that is added to or removed from a step, though PostHog masks it', async () => {
        const standIn = await startStandIn({
            inject: (row) => {
                const actions = row.actions as { id: string; config: { inputs?: Record<string, unknown> } }[]
    ...
    it('redacts resolved secrets in one pass', async () => {
        const standIn = await startStandIn({
            refuseWrites: {
                status: 400,
                body: {
...
`,
    ...
    it('refuses a push from a path the workflow was not pushed from', async () => {
        const standIn = await startStandIn()
        onTestFinished(() => standIn.close())
        standIn.seed({
            key: 'onboarding-nudge',
    ...
    it('records the new path on a move that changes nothing else', async () => {
        const standIn = await startStandIn()
        onTestFinished(() => standIn.close())
        const workspace = makeWorkspace({ 'flows/onboarding.ts': workflowFile() })
        const env = {
    ...
    it('refuses rather than adopting a row PostHog returned without the key', async () => {
    ...
    it('refuses a redirect without following it with the API key', async () => {
    ...
    it('includes the backend field name when PostHog refuses a workflow definition', async () => {
        const standIn = await startStandIn({
    ...
    it('uses API key in the permission fix', async () => {
    ...
    it('reports a write response without a workflow id as invalid', async () => {
    ...
    it('reports unparseable write success as an invalid response with an unknown outcome', async () => {
    ...
    it('sees an edit to a value whose own key has a name PostHog also uses', async () => {
        const standIn = await startStandIn()
        onTestFinished(() => standIn.close())
...
`
    ...
    it('refuses a push that cannot tell which file it comes from', async () => {
        const standIn = await startStandIn()
        onTestFinished(() => standIn.close())
    ...
    it('prints what already landed when a later workflow in the file fails', async () => {
        const standIn = await startStandIn()
        onTestFinished(() => standIn.close())
...
`,
    ...
    it('names the project it wrote to', async () => {
        const standIn = await startStandIn()
        onTestFinished(() => standIn.close())
        const workspace = makeWorkspace({ 'flows/onboarding.ts': workflowFile() })
    ...
    it('records the branch and no commit when a pull request build pushes', async () => {
        const standIn = await startStandIn()
        onTestFinished(() => standIn.close())
        const workspace = makeWorkspace({ 'flows/onboarding.ts': workflowFile() })
    ...
    it('reports no change when PostHog stores the definition in its own form', async () => {
        const standIn = await startStandIn({
            inject: (row) => {
