...
describe('check', () => {
    ...
    it('names what a push would change, and writes nothing', async () => {
    ...
    it('takes the project and the host from the flags over the environment', async () => {
        const standIn = await startStandIn()
        onTestFinished(() => standIn.close())
        const workspace = makeWorkspace({ 'flows/onboarding.ts': workflowFile() })
    ...
    it('says nothing about recording a commit, because check records nothing either way', async () => {
        const standIn = await startStandIn()
        onTestFinished(() => standIn.close())
        const workspace = makeWorkspace({ 'flows/onboarding.ts': workflowFile() })
        const env = {
    ...
    it('reports a workflow PostHog does not have yet as one it would create', async () => {
        const standIn = await startStandIn()
        onTestFinished(() => standIn.close())
        const workspace = makeWorkspace({ 'flows/onboarding.ts': workflowFile() })
    ...
    it('reports a list response without results as invalid', async () => {
    ...
    it('compares a readable secret only when check has the value', async () => {
        const standIn = await startStandIn()
        onTestFinished(() => standIn.close())
        const workspace = makeWorkspace({ 'flows/onboarding.ts': workflowFile({ secret: true }) })
        const env = {
    ...
    it('sends the version of the package as its user agent', async () => {
        const standIn = await startStandIn()
        onTestFinished(() => standIn.close())
        const workspace = makeWorkspace({ 'flows/onboarding.ts': workflowFile() })
