...
The PostHog features the CLI talks to, the workflow `key` and project secret API key auth on the workflows endpoint, are rolling out in PostHog.
Until they reach your PostHog instance, `check` still validates a file offline, and `push` refuses with `key_not_supported` when it cannot tell which workflow a file owns, rather than create a second copy.

## Install

```bash
npm install @posthog/workflows
```

The package needs Node.js 22.22 or later. It installs the `posthog-workflows` command, which you run with `npx`:

```bash
npx posthog-workflows init flows/onboarding.ts
```

## Write a workflow

`npx posthog-workflows init flows/onboarding.ts` writes a starter file with its key already filled in, taken from the file name.
...
```bash
npx posthog-workflows init flows/onboarding.ts    # write a starter file with its key filled in
npx posthog-workflows check flows/onboarding.ts   # print what a push would change
npx posthog-workflows push flows/onboarding.ts    # create or update every workflow in the file
```
...
`POSTHOG_CLI_API_KEY` also accepts the project's secret API key (`phs_...`) once PostHog accepts one on the workflows endpoint, which is rolling out.
...
Every push that writes also marks the workflow as managed by code (`managed_by: code`). A PostHog that does not support code-managed workflows yet drops that mark too, and the workflow stays editable in the PostHog editor.
...
There is nothing to sync back: the file is the source of truth, and the next push wins over any edit made in the PostHog editor.

### Which CI step do I set up?

Two jobs in the repository that owns the workflow files.
On a pull request, run `npx posthog-workflows check <file>` with no secret; it validates the file offline and does not block a contributor from a fork.
On the default branch, run `npx posthog-workflows push <file>` with one secret, the API key, and the project id in the environment.
Both are plain commands, so any CI system works.
...
Revisions are never deleted, so every version stays readable, but make an older one live again with a commit: a restore made in the PostHog editor is undone by the next push.
...
```bash
pnpm turbo run build check-types lint test:unit --filter=@posthog/workflows
```

`test:unit` runs the unit tests on Vitest. The CLI tests start the built `dist/cli/main.js`, so Turbo builds the package first.
`check-types` runs `tsc` over `src/`, including `src/__tests__/types.test-d.ts`. That file holds the rules the compiler enforces rather than an assertion, so a rule that relaxes fails the type check.
The root `.oxlintrc.json` turns the `jsdoc` rules into errors for this package's `src/`, so a JSDoc block with a missing `@param` or a mistyped tag fails lint.
