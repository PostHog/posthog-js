                ...
                if (s.type === EventType.IncrementalSnapshot && s.data.source === IncrementalSource.Mutation) {
                    ...
                    if (pluginPayload?.trace.length) {
                        pluginPayload.trace = pluginPayload.trace.map((trace) => {
                            return trace.replace(/^pptr:evaluate;.*?:(\d+:\d+)/, '__puppeteer_evaluation_script__:$1')
                        })
                    }
                    if (pluginPayload?.payload.length) {
                        pluginPayload.payload = pluginPayload.payload.map((payload) => {
                            return payload.replace(/pptr:evaluate;.*?:(\d+:\d+)/g, '__puppeteer_evaluation_script__:$1')
                        })
                    }
                }
...
export async function waitForIFrameLoad(
    ...
    await page.$eval(
        ...
        async (iframe, timeout) => {
            await new Promise<void>((resolve, reject) => {
                const timeoutId = setTimeout(() => reject(new Error('Waiting for iframe load has timed out')), timeout)
    ...
    if (!frame) throw new Error('Waiting for iframe load has timed out - no frame')
    return frame
}
