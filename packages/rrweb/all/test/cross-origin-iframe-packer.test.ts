import { describe, it, vi, beforeAll, beforeEach, afterEach, afterAll } from 'vitest'
import type { eventWithTime, listenerHandler, mutationData } from '@posthog/rrweb-types'
...
async function injectRecordScript(frame: puppeteer.Frame, options?: ExtraOptions) {
  ...
  `
        ...
        describe('should support packFn option in record()', () => {
            it('', async () => {
                ...
                const packedSnapshots = (await ctx.page.evaluate('window.snapshots')) as string[]
                const unpackedSnapshots = packedSnapshots.map((packed) => unpack(packed)) as eventWithTime[]
