    ...
    try {
        ...
        await frame.evaluate(() => {
            ...
            ;(async () => {
                ...
                window.plugin = new rrwebPluginCanvasWebRTCRecord.RRWebPluginCanvasWebRTCRecord({
...
async function startReplay(page, serverURL, recordedPage) {
    ...
    return page.evaluate(() => {
        window.plugin = new rrwebPluginCanvasWebRTCReplay.RRWebPluginCanvasWebRTCReplay({
...
void (async () => {
    ...
    async function record(url) {
        ...
        replayerPage.on('console', (msg) => console.log('REPLAY PAGE LOG:', msg.text()))
        recordedPage.on('console', (msg) => console.log('RECORD PAGE LOG:', msg.text()))
        await startReplay(replayerPage, serverURL, recordedPage)

        await Promise.all([resizeWindow(recordedPage, 0, 0, 800, 800), resizeWindow(replayerPage, 0, 800, 800, 800)])
        ...
        emitter.once('done', async () => {
            const pages = [...(await recordingBrowser.pages()), ...(await replayingBrowser.pages())]
            await server.close()
