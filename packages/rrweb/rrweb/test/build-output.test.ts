...
const javascriptArtifacts = ['rrweb.js', 'rrweb.cjs', 'rrweb.umd.cjs', 'rrweb.umd.min.cjs']
...
describe('build output', () => {
    ...
    it('emits valid source maps referenced by every JavaScript artifact', () => {
        javascriptArtifacts.forEach((artifact) => {
            ...
            expect(source.trimEnd()).toMatch(new RegExp(`sourceMappingURL=${artifact.replaceAll('.', '\\.')}.map$`))
            expect(map).toMatchObject({ version: 3 })
            expect(map.sources).not.toHaveLength(0)
            expect(map.mappings).not.toHaveLength(0)
            expect([undefined, artifact]).toContain(map.file)
        })
    })

    it('emits declaration, CSS, and bundle-analysis artifacts', () => {
        expect(readFileSync(resolve(dist, 'rrweb.d.cts'))).toEqual(readFileSync(resolve(dist, 'rrweb.d.ts')))
        ;['style.css', 'style.min.css', 'style.min.css.map'].forEach((artifact) => {
            expect(existsSync(resolve(dist, artifact))).toBe(true)
        })
        expect(existsSync(resolve(__dirname, '../../rrweb-bundle-analysis.html'))).toBe(true)
    })
})
