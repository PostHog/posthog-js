...
import { startServer, launchPuppeteer, getServerURL, waitForRAF, ISuite, hideMouseAnimation, fakeGoto } from '../utils'
...
describe('video', () => {
    ...
    it('should play from the start without media events', async () => {
        await page.evaluate(`let events = ${JSON.stringify(videoPlaybackOnFullSnapshotEvents)}`)
        await page.evaluate(`
      const { Replayer } = rrweb;
      window.replayer = new Replayer(events);
    `)
