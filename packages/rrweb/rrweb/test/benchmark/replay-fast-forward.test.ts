...
const suites: Array<{
    ...
    },
    {
        ...
        eventURL: 'https://raw.githubusercontent.com/rrweb-io/benchmark-events/main/rrdom-benchmark-1.json',
        times: 3,
    },
]
...
describe('benchmark: replayer fast-forward performance', () => {
        ...
        it(
            ...
            async () => {
                if (suite.eval) suite.eventsString = await generateEvents(suite.eval)
                    ...
                    suite.eventsString = await fetchEventsWithCache(suite.eventURL, './temp')
            ...
            <script>
              window.events = ${suite.eventsString};
                    ...
                    const duration = await page.evaluate(() => {
                        const replayer = new (window as any).rrweb.Replayer((window as any).events)
     ...
     */
    async function fetchEventsWithCache(eventURL: string, cacheFolder: string): Promise<string> {
