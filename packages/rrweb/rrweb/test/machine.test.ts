import { createPlayerService, discardPriorSnapshots } from '../src/replay/machine'
...
const events = sampleEvents.filter((e) => ![EventType.DomContentLoaded, EventType.Load].includes(e.type))
...
describe('get last session', () => {
    ...
    it('will return last session when there is more than one in the events', () => {
        ...
        expect(discardPriorSnapshots(multiple, nextNextEvents[nextNextEvents.length - 1].timestamp)).toEqual(
            nextNextEvents
        )
    })
    ...
    it('will return last session when baseline time is future time', () => {
        const multiple = events.concat(nextEvents).concat(nextNextEvents)
        expect(discardPriorSnapshots(multiple, nextNextEvents[nextNextEvents.length - 1].timestamp + 1000)).toEqual(
            nextNextEvents
        )
    })

    it('will return all sessions when baseline time is prior time', () => {
        expect(discardPriorSnapshots(events, events[0].timestamp - 1000)).toEqual(events)
    })
})
