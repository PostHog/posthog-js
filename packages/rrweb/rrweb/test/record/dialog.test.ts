...
import { assertSnapshot, getServerURL, ISuite, launchPuppeteer, startServer, waitForRAF } from '../utils'
import { attributeMutation, EventType, eventWithTime, listenerHandler } from '@posthog/rrweb-types'
...
const attributeMutationFactory = (mutation: attributeMutation['attributes']) => {
    return {
        data: {
            attributes: [
                {
...
describe('dialog', () => {
    ...
    it('showModal dialog', async () => {
        ...
        expect(lastEvent).toMatchObject(attributeMutationFactory({ rr_open_mode: 'modal' }))
    })
