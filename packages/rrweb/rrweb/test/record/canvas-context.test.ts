...
describe('initCanvasContextObserver', () => {
    ...
    it('adds the snapshot-safe usage flags when webgpu contexts are configured', () => {
        ...
        expect((canvas as FakeCanvasElement & { __context?: string }).__context).toBe('webgpu')
        expect(getContextSpy).toHaveBeenCalledWith('webgpu')
        expect(canvas.__lastWebGPUConfigure).toEqual({
            usage: textureUsage.TEXTURE_BINDING | textureUsage.COPY_SRC | textureUsage.RENDER_ATTACHMENT,
        })
    ...
    it('leaves other contexts unchanged', () => {
        ...
        const getContext = vi.spyOn(FakeCanvasElement.prototype, 'getContext').mockReturnValue({} as RenderingContext)
        const restore = initCanvasContextObserver(
            win as unknown as Parameters<typeof initCanvasContextObserver>[0],
            'rr-block',
            null,
        ...
        expect((canvas as FakeCanvasElement & { __context?: string }).__context).toBe('webgl')
