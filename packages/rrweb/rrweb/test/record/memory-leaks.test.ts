...
import { findAndRemoveIframeBuffer, mutationBuffers } from '../../src/record/observer'
...
describe('memory leak prevention', () => {
    ...
    describe('IframeManager cleanup', () => {
        ...
        it('should clear WeakMaps to prevent iframe memory leaks', () => {
            ...
            const newWeakMapsCreated = weakMapConstructorCalls - weakMapCallsAfterRecord
            ...
            ({ shouldAppendToDOM, shouldCallRemove, expectedMapsCleanedUp }) => {
                ...
                if (shouldAppendToDOM) {
                    ...
                    if (iframe.contentWindow) {
                        expect(manager.crossOriginIframeMap.has(iframe.contentWindow)).toBe(true)
                    }
                }
                ...
                if (expectedMapsCleanedUp) {
                    ...
                    if (iframe.contentWindow) {
                        expect(manager.crossOriginIframeMap.has(iframe.contentWindow)).toBe(false)
                    }
    ...
    describe('IframeManager cross-origin SecurityError handling', () => {
        ...
        it.each([{ method: 'destroy' as const }, { method: 'removeIframeById' as const }])(
        ...
        it.each([{ method: 'destroy' as const }, { method: 'removeIframeById' as const }])(
            ...
            ({ method }) => {
                ...
                if (method === 'destroy') {
                    ...
                    manager.nestedIframeListeners.set(badWin, vi.fn())
                    expect(() => iframeManager.removeIframeById(iframeId)).toThrow(TypeError)
                    document.body.removeChild(iframe)
                }
            }
        )
