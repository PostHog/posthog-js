...
function addNativeLikeSetter(
  prototype: object,
  property: string,
  prototypeValue: object,
) {
...
describe('canvas setter blocking', () => {
  it('does not record blocked 2D setter fallback mutations', async () => {
    ...
    const visible = new Fake2DContext(
  ...
  it('does not record blocked or offscreen WebGL setter fallback mutations', async () => {
    class FakeWebGLContext {
      constructor(public canvas: FakeCanvas | object | null) {}
    }
    addNativeLikeSetter(
      FakeWebGLContext.prototype,
      'lineWidth',
      FakeWebGLContext.prototype,
    );
    ...
    const blocked = new FakeWebGLContext(
      new FakeCanvas(true),
    ) as FakeWebGLContext & {
      lineWidth: number;
    };
    const visible = new FakeWebGLContext(
      new FakeCanvas(false),
    ) as FakeWebGLContext & {
      lineWidth: number;
    };
    ...
    const detached = new FakeWebGLContext(null) as FakeWebGLContext & {
      lineWidth: number;
    };
