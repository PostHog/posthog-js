...
describe('StylesheetManager.inlineDeferredLinkElement()', () => {
    ...
    const emittedCssText = () => mutationCb.mock.calls[0][0].attributes[0].attributes._cssText as string

    const makeManager = (options: Partial<ConstructorParameters<typeof StylesheetManager>[0]> = {}) =>
    ...
    it('masks the emitted _cssText under maskAllElementAttributes', () => {
        makeManager({ maskAllElementAttributes: true }).inlineDeferredLinkElement(linkEl, LINK_ID)
    ...
    it('routes the emitted _cssText through maskAttributeFn', () => {
        ...
        makeManager({ maskAttributeFn }).inlineDeferredLinkElement(linkEl, LINK_ID)

        expect(emittedCssText()).toBe('[CSS-MASKED]')
        expect(maskAttributeFn).toHaveBeenCalledWith('_cssText', expect.stringContaining('alice@example.com'), linkEl)
    })
})
