...
describe('StylesheetManager.beginDeferredLinkInlining()', () => {
  ...
  it('discards the emit and counts a failure when the sheet is swapped mid-slicing', () => {
    const swapLink = document.createElement('link');
    document.head.appendChild(swapLink);
    Object.defineProperty(swapLink, 'sheet', {
      value: styleEl.sheet,
      configurable: true,
    });
    ...
    try {
      ...
      task.advance(RULES_PER_SLICE);
      // an SPA swaps the link's href, replacing its sheet, while we slice
      Object.defineProperty(swapLink, 'sheet', {
        value: replacementStyle.sheet,
        configurable: true,
      });
      while (!task.advance(RULES_PER_SLICE)) {
        // drain
      }
      // emitting the old sheet's CSS would override the new stylesheet in
      // replay; the new sheet is handled by its own mutation flow
      expect(mutationCb).not.toHaveBeenCalled();
  ...
  it('discards the emit and counts a failure when the captured sheet is mutated mid-slicing', () => {
    ...
    // the recorder's observer saw insertRule() on the deferred link's sheet:
    // that mutation is recorded as its own event, which the defer-time
    // _cssText would overwrite in replay
    manager.onCssomSheetMutation(linkEl.sheet);
  ...
  it('keeps emitting when an unrelated sheet is mutated mid-slicing', () => {
    ...
    try {
      ...
      // some other sheet mutating (an imported sheet included: it is a
      // different CSSStyleSheet object than the captured root) is not a
      // reason to drop this link's defer-time text
      manager.onCssomSheetMutation(otherStyle.sheet);
  ...
  it('ignores mutations that arrive after the sheet finished emitting', () => {
    ...
    // post-emit mutations replay on top of the emitted _cssText, exactly as
    // they do for a sheet inlined synchronously; nothing to invalidate
    manager.onCssomSheetMutation(linkEl.sheet);
    expect(getDeferredStylesheetStats().failedCount).toBe(0);
  });
