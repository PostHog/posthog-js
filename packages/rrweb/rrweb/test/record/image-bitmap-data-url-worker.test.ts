...
describe('image-bitmap-data-url-worker', () => {
  ...
  it('records the frame as is when the provider reports nothing to mask', async () => {
    const onmessage = await loadWorker();
  ...
  it('keyframes only the canvas whose interval has elapsed', async () => {
    vi.useFakeTimers({ toFake: ['Date'] });
    try {
      const onmessage = await loadWorker();
      const mask = [{ x: 0, y: 0, width: 1, height: 1 }];
      ...
      await onmessage(frame(2, CONTENT_A, WIDTH, HEIGHT, mask));
      ...
      expect(postMessage.mock.calls[1][0]).toEqual({ id: 2 });
    } finally {
      vi.useRealTimers();
    }
  });
