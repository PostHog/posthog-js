...
describe('inlineDeferredStylesheets()', () => {
  ...
  it('flush() stops at the safety cap, skipping sheets that cannot finish so later ones still emit', () => {
    // 50 flush slices cover 10,000 rules: sheet A (31 slices incl. completion)
    // finishes; sheet B needs another 31, which cannot fit the remaining 19,
    // so it is abandoned up front; sheet C then fits easily
    ...
    expect(mutationCb).toHaveBeenCalledTimes(2);
    expect(mutationCb.mock.calls.map((call) => call[0].attributes[0].id)).toEqual(
      [1, 3],
    );
    expect(getDeferredStylesheetStats().abandonedCount).toBe(1);
    expect(onDone).toHaveBeenCalledTimes(1);
    expect(scheduled.size).toBe(0);
  });

  it('flush() abandons an oversized head-of-queue sheet up front so cheap sheets are salvaged', () => {
    // sheet A cannot finish inside the whole 50-slice cap: spending budget on
    // it would emit nothing while starving B and C, which cost one slice each
    const linkA = makeLink(12_000, 1);
    const linkB = makeLink(30, 2);
    const linkC = makeLink(30, 3);
    const onDone = vi.fn();
  ...
  it('flush() re-checks the size estimate as frames open, catching rules hidden in grouping rules', () => {
    // the sheet looks like one rule until its @media frame opens and reveals
    // 15,000 children - far beyond the cap; the per-slice re-check abandons it
    // and the small sheet behind it still emits
    const children = Array.from({ length: 15_000 }, (_, i) => ({
      cssText: `.hidden-${i} { color: red }`,
    }));
    const mediaRule = {
      cssRules: children,
      media: { mediaText: '(min-width: 1px)' },
      get cssText(): string {
    ...
    const bigLink = document.createElement('link');
    document.head.appendChild(bigLink);
    Object.defineProperty(bigLink, 'sheet', { value: sheet });
    record.mirror.add(bigLink, {
      type: NodeType.Element,
      tagName: 'link',
      attributes: {},
      childNodes: [],
    ...
    cleanupNodes.push(bigLink);
    const smallLink = makeLink(30, 2);
    const onDone = vi.fn();
    ...
    expect(mutationCb).toHaveBeenCalledTimes(1);
    expect(mutationCb.mock.calls[0][0].attributes[0].id).toBe(2);
    expect(getDeferredStylesheetStats().abandonedCount).toBe(1);
    expect(onDone).toHaveBeenCalledTimes(1);
  });

  it('a cancel() re-entered from a sheet\'s own emit stops the tick without further emits or onDone', () => {
    // real path: posthog-js emits synchronously, and an emitted mutation can
    // trigger a full snapshot, whose rebuild cancels this queue mid-tick
    const linkA = makeLink(50, 1);
    const linkB = makeLink(50, 2);
    const onDone = vi.fn();
    ...
    // B must never be emitted by the superseded tick, and completion
    // signalling belongs to whoever cancelled
    expect(mutationCb).toHaveBeenCalledTimes(1);
    expect(onDone).not.toHaveBeenCalled();
    expect(scheduled.size).toBe(0);
  });

  it('a flush() re-entered from a sheet\'s final emit emits that sheet exactly once', () => {
    const linkA = makeLink(50, 1);
    const linkB = makeLink(50, 2);
    const onDone = vi.fn();
    ...
    // A once (the emit that re-entered), B once (drained by the inner flush)
    expect(mutationCb).toHaveBeenCalledTimes(2);
    expect(mutationCb.mock.calls[0][0].attributes[0].id).toBe(1);
    expect(mutationCb.mock.calls[1][0].attributes[0].id).toBe(2);
    expect(onDone).toHaveBeenCalledTimes(1);
  ...
  it('contains a throwing emit in the idle loop and keeps draining the queue', () => {
    const linkA = makeLink(50, 1);
    const linkB = makeLink(50, 2);
    const onDone = vi.fn();
    mutationCb.mockImplementationOnce(() => {
    ...
    // must not surface as an uncaught error from the idle callback
    expect(() =>
      fireIdle({ didTimeout: false, timeRemaining: () => 50 }),
    ).not.toThrow();

    // A's emit threw and is counted; B still shipped
    expect(mutationCb).toHaveBeenCalledTimes(2);
    expect(mutationCb.mock.calls[1][0].attributes[0].id).toBe(2);
    expect(getDeferredStylesheetStats().abandonedCount).toBe(1);
    expect(onDone).toHaveBeenCalledTimes(1);
  ...
  it('contains a throwing emit inside flush() and still drains the sheets behind it', () => {
    const linkA = makeLink(50, 1);
    const linkB = makeLink(50, 2);
    const onDone = vi.fn();
    mutationCb.mockImplementationOnce(() => {
...
describe('record() teardown of deferred stylesheets', () => {
  ...
  it('stop() completes teardown even when an emit throws during the teardown flush', () => {
    makeUnregisteredLink(1000, 'stop-throwing');
    let throwOnMutation = false;
    stop = record({
      emit: (event) => {
        const typed = event as eventWithTime;
        if (
          throwOnMutation &&
          typed.type === EventType.IncrementalSnapshot &&
          typed.data.source === IncrementalSource.Mutation
        ) {
          throw new Error('emit failed');
        }
        events.push(typed);
    ...
    // teardown ran to completion: recording is off...
    expect(() => record.addCustomEvent('tag', {})).toThrow();
    // ...and the handlers (the suspension listeners among them) were removed
    const generation = getSuspensionGeneration();
    document.dispatchEvent(new Event('visibilitychange'));
    expect(getSuspensionGeneration()).toBe(generation);
  });

  it('pagehide contains a throwing emit instead of surfacing it on the page', () => {
    makeUnregisteredLink(1000, 'pagehide-throwing');
    let throwOnMutation = false;
    stop = record({
      emit: (event) => {
        const typed = event as eventWithTime;
  ...
  it('cancels a pending deferral when its sheet is mutated via insertRule mid-deferral', () => {
    const linkEl = makeUnregisteredLink(1000, 'mutated');
    stop = startRecording();
    ...
    // the defer-time text would overwrite the recorded insertRule in replay,
    // so the deferral is dropped and counted; the link keeps its href
    expect(cssTextMutations(events)).toHaveLength(0);
  ...
  it('cancels a pending deferral when a rule declaration is mutated via setProperty mid-deferral', () => {
    const linkEl = makeUnregisteredLink(1000, 'declared');
    stop = startRecording();
    ...
    window.CSSStyleDeclaration.prototype.setProperty.call(
      firstRule.style,
      'color',
      'green',
    );
...
describe('record() end-to-end delivery of deferred stylesheets', () => {
  ...
  it('keeps the replacement queue flushable when a full snapshot is taken from inside a deferred emit', () => {
    // real path: posthog-js calls emit synchronously and can take a full
    // snapshot from within it (URL-trigger resume, checkout, heal); the
    // superseded queue's tick must not clobber the new queue's handle, or
    // stop()/pagehide would silently flush nothing for it
    makeExternalLink(50, 'trigger');
    makeExternalLink(600, 'survivor');
    let retriggered = false;
    stop = record({
      emit: (event) => {
        events.push(event as eventWithTime);
        const pushed = event as eventWithTime;
        if (
          !retriggered &&
          pushed.type === EventType.IncrementalSnapshot &&
          pushed.data.source === IncrementalSource.Mutation &&
          (pushed.data.attributes ?? []).some(
            (attribute) =>
              typeof (attribute.attributes as Record<string, unknown>)
                ._cssText === 'string',
          )
        ) {
          retriggered = true;
      ...
      inlineStylesheetBudgetRules: 1, // both links defer
    });

    // the trigger sheet completes in the first idle slice; its emit takes a
    // full snapshot, which cancels this queue mid-tick and installs a new one
    fireIdle({ didTimeout: false, timeRemaining: () => 0 });
    expect(retriggered).toBe(true);
    expect(fullSnapshots()).toHaveLength(2);
    const boundary = events.indexOf(fullSnapshots()[1]);
    const secondIds = collectLinks(fullSnapshots()[1]).map((link) => link.id);
    expect(secondIds).toHaveLength(2);
    ...
    const afterBoundary = cssTextAttributeMutations(events.slice(boundary));
    expect(afterBoundary).toHaveLength(2);
    expect(new Set(afterBoundary.map((m) => m.id))).toEqual(new Set(secondIds));
    expect(getDeferredStylesheetStats().abandonedCount).toBe(0);
    expect(scheduled.size).toBe(0);
  });
...
describe('record() suspension boundary wiring', () => {
  let stop: (() => void) | undefined;
  let cleanupNodes: Element[];
  ...
  it('discards duration samples straddling a document visibilitychange while recording', () => {
    stop = record({ emit: () => {} });
    ...
    expect(getMutationCost().slowestBatchMs).toBe(0);
    expect(getDiscardedDurationSamples()).toBe(1);
  });

  it('treats Page Lifecycle freeze and resume as suspension boundaries', () => {
    stop = record({ emit: () => {} });

    const before = getSuspensionGeneration();
    document.dispatchEvent(new Event('freeze'));
    document.dispatchEvent(new Event('resume'));

    expect(getSuspensionGeneration()).toBe(before + 2);
  });

  it('removes the suspension listeners when recording stops', () => {
    stop = record({ emit: () => {} });
    stop();
    stop = undefined;
    ...
    expect(getSuspensionGeneration()).toBe(before);
  });

  it('discards the full snapshot cost when visibility changes mid-snapshot', () => {
    // stringifying this link's sheet dispatches visibilitychange, modelling a
    // handler running inside the sync window via a nested event loop
    const linkEl = document.createElement('link');
    linkEl.setAttribute('rel', 'stylesheet');
    linkEl.setAttribute('href', 'http://localhost/mid-snapshot.css');
    Object.defineProperty(linkEl, 'sheet', {
      value: {
        href: 'http://localhost/mid-snapshot.css',
        cssRules: [
          {
            get cssText(): string {
              document.dispatchEvent(new Event('visibilitychange'));
              return '.mid-snapshot { color: red }';
            },
          },
        ],
      } as unknown as CSSStyleSheet,
