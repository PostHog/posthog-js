...
import type { listenerHandler, eventWithTime, mutationData } from '@posthog/rrweb-types'
import { EventType, IncrementalSource } from '@posthog/rrweb-types'
import { assertSnapshot, getServerURL, launchPuppeteer, startServer, waitForRAF, waitForIFrameLoad } from '../utils'
...
async function injectRecordScript(frame: puppeteer.Frame, options?: ExtraOptions, retryCount = 0) {
    const MAX_RETRIES = 5
    try {
        ...
        const retryableErrors = ['DOM.resolveNode', 'DOM.describeNode', 'Execution context was destroyed']
        const isRetryable = retryableErrors.some((error) => e.message.includes(error))
...
const setup = function (this: ISuite, content: string, options?: ExtraOptions): ISuite {
    const ctx = {} as ISuite & {
        serverB: http.Server
        serverBURL: string
    }
    ...
    beforeEach(async () => {
        ...
        await ctx.page.setContent(content.replace(/\{SERVER_URL\}/g, ctx.serverURL))
    ...
    describe('form.html', function (this: ISuite) {
      ...
      `
        ...
        it("won't emit events if it isn't in the top level iframe", async () => {
            const el = (await ctx.page.$('body > iframe')) as puppeteer.ElementHandle<Element>

            const frame = await el.contentFrame()
            const events = await frame?.evaluate(() => (window as unknown as IWindow).snapshots)
            expect(events).toMatchObject([])
        })

        it('will emit events if it is in the top level iframe', async () => {
            const events = await ctx.page.evaluate(() => (window as unknown as IWindow).snapshots)
        ...
        it('should use unique id for child of iframes', async () => {
            const events: eventWithTime[] = await ctx.page.evaluate(() => (window as unknown as IWindow).snapshots)
            await waitForRAF(ctx.page)
            expect((events[events.length - 1].data as mutationData).adds[0].node.id).not.toBe(1)
        })
        ...
        it('should replace the existing DOM nodes on iframe navigation with `isAttachIframe`', async () => {
            ...
            const events: eventWithTime[] = await ctx.page.evaluate(() => (window as unknown as IWindow).snapshots)
            expect((events[events.length - 1].data as mutationData).removes).toMatchObject([])
            expect((events[events.length - 1].data as mutationData).isAttachIframe).toBeTruthy()
        })
        ...
        it('should preserve iframe attachments after full snapshot', async () => {
            ...
            const initialAttachmentEvents = initialEvents.filter(
                (e) => e.type === EventType.IncrementalSnapshot && (e.data as mutationData).isAttachIframe
            )
            ...
            const fullSnapshots = eventsAfterSnapshot.filter((e) => e.type === EventType.FullSnapshot)
            ...
            expect(attachmentEventsAfter.length).toBeGreaterThan(initialAttachmentEvents.length)
        ...
        it('should map input events correctly', async () => {
            ...
            const snapshots = (await ctx.page.evaluate('window.snapshots')) as eventWithTime[]
    ...
    describe('move-node.html', function (this: ISuite) {
      ...
      `
        ...
        it('captures mutations on adopted stylesheets', async () => {
            ...
            await waitForRAF(ctx.page)
            await ctx.page.evaluate(() => {
                document.adoptedStyleSheets![0].replaceSync!('div { display: inline ; }')
            })
            await frame.evaluate(() => {
                document.adoptedStyleSheets![0].replaceSync!('h1 { font-size: large; }')
            })
            await waitForRAF(ctx.page)
            await ctx.page.evaluate(() => {
                ;(document.adoptedStyleSheets![0].cssRules[0] as CSSStyleRule).style.setProperty('color', 'green')
                ;(document.adoptedStyleSheets![0].cssRules[0] as CSSStyleRule).style.removeProperty('display')
            })
            await frame.evaluate(() => {
                ;(document.adoptedStyleSheets![0].cssRules[0] as CSSStyleRule).style.setProperty(
                    'font-size',
                    'medium',
                    'important'
                )
            ...
            await waitForRAF(ctx.page)
            await ctx.page.evaluate(() => {
                document.adoptedStyleSheets![0].insertRule('body { border: 2px solid blue; }', 1)
            })
            await frame.evaluate(() => {
        ...
        it('captures mutations on stylesheets', async () => {
            ...
            await waitForRAF(ctx.page)
            await ctx.page.evaluate(() => {
                ;(document.styleSheets[0].cssRules[0] as CSSStyleRule).style.setProperty('color', 'green')
                ;(document.styleSheets[0].cssRules[0] as CSSStyleRule).style.removeProperty('display')
            })
            await frame.evaluate(() => {
            ...
            await waitForRAF(ctx.page)
            await ctx.page.evaluate(() => {
                document.styleSheets[0].insertRule('body { border: 2px solid blue; }', 1)
            })
            await frame.evaluate(() => {
        ...
        it('should filter out forwarded cross origin rrweb messages', async () => {
            ...
            await frame.evaluate((iframe2URL) => {
                ...
                window.addEventListener('message', (event) => {
                    if (event.source !== window) window.parent.postMessage(event.data, '*')
                })
                const iframe2 = document.createElement('iframe')
...
describe('same origin iframes', function (this: ISuite) {
    ...
    it('should clean up iframe from attachedIframes Map when removed from DOM', async () => {
        ...
        const initialEvents: eventWithTime[] = await ctx.page.evaluate(() => (window as unknown as IWindow).snapshots)
        ...
        const eventsAfterAdd: eventWithTime[] = await ctx.page.evaluate(() => (window as unknown as IWindow).snapshots)
        ...
        const addMutations = eventsAfterAdd.filter(
            (e) =>
                ...
                (e.data as mutationData).adds?.some((add) => add.node.tagName === 'iframe')
        )
