...
import { eventWithTime, EventType, IncrementalSource, listenerHandler, mutationData } from '@posthog/rrweb-types'
...
describe('record: attribute masking', () => {
    ...
    beforeAll(async () => {
        ...
        code = fs.readFileSync(path.resolve(__dirname, '../../dist/rrweb.umd.cjs'), 'utf8')
    })
    ...
    `)
        ...
        await page.exposeFunction('emit', (event: eventWithTime) => {
            if (event.type !== EventType.DomContentLoaded && event.type !== EventType.Load) {
                events.push(event)
            }
        })
    })
    ...
    it('does not let compact styleDiff overwrite a callback mask', async () => {
        await page.evaluate(() => {
            ;(window as unknown as IWindow).rrweb.record({
                ...
                maskAttributeFn: (name, value) => (name === 'style' ? '[STYLE-MASKED]' : value),
            })
        })
        ...
        await page.evaluate(() => {
            document
                ...
                .setAttribute('style', 'color: blue; background-color: red; border-color: green; outline-color: black;')
        })
        await waitForRAF(page)
    ...
    it('uses qualified names for SVG namespace mutations and preserves removals', async () => {
        await page.evaluate(() => {
            ;(window as unknown as IWindow).rrweb.record({
                emit: (window as unknown as IWindow).emit,
                maskAttributeFn: (name, value, element) => (element instanceof SVGElement ? `[${name}-MASKED]` : value),
            })
        })
        await waitForRAF(page)
        events = []
        ...
        await page.evaluate(() => {
            document
                .getElementById('icon')!
                .setAttributeNS('http://www.w3.org/1999/xlink', 'xlink:href', '/sprites.svg#alice@example.com')
        })
        await waitForRAF(page)
        ...
        events = []
        await page.evaluate(() => {
            document.getElementById('icon')!.removeAttributeNS('http://www.w3.org/1999/xlink', 'href')
        })
        await waitForRAF(page)
    ...
    it('masks an inaccessible iframe mutation under rr_src', async () => {
        await page.evaluate(() => {
            ;(window as unknown as IWindow).rrweb.record({
                emit: (window as unknown as IWindow).emit,
                maskAttributeFn: (name, value) => (name === 'rr_src' ? '[IFRAME-SOURCE-MASKED]' : value),
            })
        })
        await waitForRAF(page)
        events = []
        ...
        await page.evaluate(() => {
            document.getElementById('frame')!.setAttribute('src', 'https://example.com/?email=alice@example.com')
        })
        await waitForRAF(page)
