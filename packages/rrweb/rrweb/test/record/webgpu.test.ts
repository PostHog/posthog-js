...
import { listenerHandler, eventWithTime, EventType, IncrementalSource, CanvasContext } from '@posthog/rrweb-types'
...
const setup = function (this: ISuite, content: string): ISuite {
    ...
    beforeEach(async () => {
        ...
        await ctx.page.evaluate(() => {
            ...
            ).__preExistingWebGPUContext = canvas.getContext('webgpu') as unknown as FakeGPUCanvasContext
            ...
            window.createImageBitmap = async (source) => {
                ...
                if (sourceCanvas.__fakeWebGPUContext) {
                    const requiredUsage = GPUTextureUsage.COPY_SRC | GPUTextureUsage.RENDER_ATTACHMENT
                    const configuredUsage = sourceCanvas.__lastWebGPUConfigure?.usage

                    if (typeof configuredUsage !== 'number' || (configuredUsage & requiredUsage) !== requiredUsage) {
