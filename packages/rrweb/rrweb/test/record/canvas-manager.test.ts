...
vi.mock('../../src/record/workers/image-bitmap-data-url-worker?worker&inline', () => ({
    default: class {
        ...
        constructor() {
            if (workerControl.throwOnConstruct) {
                throw new DOMException("Failed to execute 'importScripts' on 'WorkerGlobalScope'", 'NetworkError')
            }
...
describe('CanvasManager FPS observer', () => {
    ...
    it('should recover from createImageBitmap errors', async () => {
        ...
        vi.stubGlobal('createImageBitmap', vi.fn().mockRejectedValueOnce(new Error('GPU context lost')))
...
describe('CanvasManager reference-counted teardown', () => {
    const makeManager = (): {
        ...
        ;(manager as unknown as { resetObservers: () => void }).resetObservers = resetObservers
