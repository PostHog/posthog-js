...
import { EventType, IncrementalSource, ReplayerEvents } from '@posthog/rrweb-types'
...
type IWindow = Window & typeof globalThis & { rrweb: typeof import('../src'); events: typeof events }
...
describe('replayer', function () {
    ...
    it('can play at any time offset', async () => {
      ...
      replayer['timer']['actions'].length;
    `)
        expect(actionLength).toEqual(events.filter((e) => e.timestamp - events[0].timestamp >= 1500).length)
    })
    ...
    it('can play a second time to the past', async () => {
      ...
      replayer['timer']['actions'].length;
    `)
        expect(actionLength).toEqual(events.filter((e) => e.timestamp - events[0].timestamp >= 500).length)
    })
    ...
    `)
        ...
        expect(await contentDocument!.$eval('#container', (element: Element) => element.scrollTop)).toEqual(0)
        ...
        expect(await contentDocument!.$eval('#container', (element: Element) => element.scrollTop)).toEqual(2500)
        ...
        expect(
            await page.$eval('iframe', (element: Element) => (element as HTMLIFrameElement)!.contentWindow!.scrollY)
        ).toEqual(250)
    ...
    `)
        ...
        await page.evaluate('replayer.pause(1050);')
        // scroll the ".container" div' at 1000
        expect(await contentDocument!.$eval('.container', (element: Element) => element.scrollTop)).toEqual(800)
    ...
    `)
        ...
        const iframe = await page.$('iframe')
        const contentDocument = await iframe!.contentFrame()!
        expect(await contentDocument!.$eval('#sheet', (element: Element) => element.scrollTop)).toEqual(1320)
    })
    ...
    `)
        ...
        const iframe = await page.$('iframe')
        const contentDocument = await iframe!.contentFrame()!
        expect(await contentDocument!.$eval('html', (element: Element) => element.scrollTop)).toEqual(800)
    })
    ...
    `)
        const iframe = await page.$('iframe')
        const contentDocument = await iframe!.contentFrame()!
        expect(await contentDocument!.$eval('#reveal-container', (element: Element) => element.scrollTop)).toEqual(0)
    })
    ...
    `)
        const iframe = await page.$('iframe')
        const contentDocument = await iframe!.contentFrame()!
        expect(await contentDocument!.$eval('#reveal-container', (element: Element) => element.scrollTop)).toEqual(787)
    })
    ...
    `)
        ...
        expect(
            await contentDocument!.$eval('select', (element: Element) => (element as HTMLSelectElement).value)
        ...
        expect(
            await contentDocument!.$eval('input', (element: Element) => (element as HTMLSelectElement).value)
        ).toEqual('')
    ...
    `)
        ...
        let iframeOneDocument = await (await contentDocument!.$('iframe'))!.contentFrame()
        ...
        expect(
            await iframeOneDocument!.$eval('noscript', (element) => window.getComputedStyle(element).display)
        ).toEqual('none')
        ...
        iframeOneDocument = await (await contentDocument!.$('iframe'))!.contentFrame()
        expect((await iframeOneDocument!.$$('style')).length).toBe(1)

        expect((await contentDocument!.$$('iframe')).length).toEqual(2)
        let iframeTwoDocument = await (await contentDocument!.$$('iframe'))[1]!.contentFrame()
        ...
        const iframeThreeDocument = await (await iframeTwoDocument!.$$('iframe'))[0]!.contentFrame()
        let iframeFourDocument = await (await iframeTwoDocument!.$$('iframe'))[1]!.contentFrame()
        ...
        await waitForRAF(page)
        await page.evaluate('replayer.pause(1550);')
        iframeTwoDocument = await (await contentDocument!.$$('iframe'))[1]!.contentFrame()
        expect((await iframeTwoDocument!.$$('style')).length).toBe(1)
        iframeFourDocument = await (await iframeTwoDocument!.$$('iframe'))[1]!.contentFrame()
        ...
        const iframeFiveDocument = await (await iframeFourDocument!.$('iframe'))!.contentFrame()
        ...
        expect(
            await iframeFiveDocument!.$eval('noscript', (element) => window.getComputedStyle(element).display)
        ).toEqual('none')
    ...
    `)
        ...
        const iframe = await page.$('iframe')
        const contentDocument = await iframe!.contentFrame()!
        expect(await contentDocument!.$eval('div', (element) => element.shadowRoot)).not.toBeNull()
        expect(
            await contentDocument!.evaluate(
                () => document.querySelector('body > div')!.shadowRoot!.querySelector('span')!.textContent
            )
        ...
        expect(
            await contentDocument!.evaluate(
                () => document.querySelector('body > div')!.shadowRoot!.querySelector('div')!.shadowRoot
            )
        ).not.toBeNull()
        expect(
            await contentDocument!.evaluate(
    ...
    `)
        ...
        const checkCorrectness = async () => {
            ...
            expect(await contentDocument!.$eval('div', (element) => window.getComputedStyle(element).color)).toEqual(
                colorRGBMap.yellow
            )
                ...
                await contentDocument!.evaluate(
                    () =>
                        window.getComputedStyle(
                            document.querySelector('#shadow-host1')!.shadowRoot!.querySelector('span')!
                        ).color
                )
            ...
            expect(
                await contentDocument!.$eval(
                    'iframe',
                    (element) =>
                        window.getComputedStyle((element as HTMLIFrameElement).contentDocument!.querySelector('h1')!)
                            .color
                )
            ...
            expect(
                await contentDocument!.evaluate(
                    () =>
                        window.getComputedStyle(
                            document.querySelector('#shadow-host2')!.shadowRoot!.querySelector('span')!
                        ).color
                )
      ...
      return new Promise((resolve) => {
        ...
        const check250ms = async () => {
            expect(
                await contentDocument!.evaluate(
                    () =>
                        document.adoptedStyleSheets.length === 1 && document.adoptedStyleSheets[0].cssRules.length === 0
                )
            ).toBeTruthy()
            expect(
                await contentDocument!.evaluate(
                    () =>
                        document.querySelector('iframe')!.contentDocument!.adoptedStyleSheets.length === 1 &&
                        document.querySelector('iframe')!.contentDocument!.adoptedStyleSheets[0].cssRules.length === 0
                )
            ).toBeTruthy()
        }
            ...
            expect(
                await contentDocument!.evaluate(
                    () =>
                        ...
                        document.adoptedStyleSheets[0].cssRules[0].cssText === 'div { color: yellow; }'
                )
            ).toBeTruthy()
            expect(
                await contentDocument!.evaluate(
                    () =>
                        document.querySelector('iframe')!.contentDocument!.adoptedStyleSheets[0].cssRules.length ===
                            1 &&
                        document.querySelector('iframe')!.contentDocument!.adoptedStyleSheets[0].cssRules[0].cssText ===
                            'h1 { color: blue; }'
                )
            ).toBeTruthy()
        }
        ...
        const check400ms = async () => {
            expect(
                await contentDocument!.evaluate(
                    () =>
                        document.adoptedStyleSheets[0].cssRules.length === 1 &&
                        document.adoptedStyleSheets[0].cssRules[0].cssText === 'div { display: inline; }'
                )
            ).toBeTruthy()
            expect(
                await contentDocument!.evaluate(
        ...
        const check500ms = async () => {
            expect(
                await contentDocument!.evaluate(
                    () =>
                        document.adoptedStyleSheets[0].cssRules.length === 1 &&
                        document.adoptedStyleSheets[0].cssRules[0].cssText === 'div { color: green; }'
                )
            ).toBeTruthy()
            expect(
                await contentDocument!.evaluate(
                    () =>
                        ...
                        document.querySelector('iframe')!.contentDocument!.adoptedStyleSheets[0].cssRules[0].cssText ===
                            'h2 { color: red; }' &&
                        document.querySelector('iframe')!.contentDocument!.adoptedStyleSheets[0].cssRules[1].cssText ===
            ...
            expect(
                await contentDocument!.evaluate(
                    () =>
                        ...
                        document.adoptedStyleSheets[0].cssRules[0].cssText === 'div { color: green; }' &&
                        document.adoptedStyleSheets[0].cssRules[1].cssText === 'body { border: 2px solid blue; }'
                )
            ).toBeTruthy()
            expect(
                await contentDocument!.evaluate(
    ...
    it('should replay document replacement events without warnings or errors', async () => {
        await page.evaluate(`events = ${JSON.stringify(documentReplacementEvents)}`)
    ...
    `)
        ...
        expect(await docInIFrame?.evaluate(() => document.querySelector('span')?.className)).toBe(':hover')
        ...
        expect(await docInIFrame?.evaluate(() => document.querySelector('span')?.className)).toBe('')
        expect(
            await docInIFrame?.evaluate(() => {
                const shadowRoot = document.querySelector('div')?.shadowRoot
                return (shadowRoot?.childNodes[0] as HTMLElement).className
