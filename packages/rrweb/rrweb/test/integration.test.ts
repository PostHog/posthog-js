...
import { eventWithTime, NodeType, EventType, IncrementalSource } from '@posthog/rrweb-types'
import { visitSnapshot } from '@posthog/rrweb-snapshot'

describe('record integration tests', function (this: ISuite) {
    vi.setConfig({ testTimeout: 10_000 })

    const getHtml = (fileName: string, options: recordOptions<eventWithTime> = {}): string => {
    ...
    it('can record style changes compactly and preserve css var() functions', async () => {
        ...
        await page.evaluate('document.body.setAttribute("style", "background: var(--mystery)")')
        ...
        await page.evaluate('document.body.setAttribute("style", "color:var(--mystery-color);display:block")')
        await waitForRAF(page)
        ...
        await page.evaluate('document.body.style.marginTop = 0; document.body.style.color = null')
        await waitForRAF(page)
    ...
    it('can freeze mutations', async () => {
        const page: puppeteer.Page = await browser.newPage()
        await page.goto('about:blank')
        await page.setContent(getHtml.call(this, 'mutation-observer.html', { recordCanvas: true }))
    ...
    it('should not record input values if maskAllInputs is enabled', async () => {
        const page: puppeteer.Page = await browser.newPage()
        await page.goto('about:blank')
        await page.setContent(getHtml.call(this, 'form.html', { maskAllInputs: true }))
    ...
    it('should record input userTriggered values if userTriggeredOnInput is enabled', async () => {
        const page: puppeteer.Page = await browser.newPage()
        await page.goto('about:blank')
        await page.setContent(getHtml.call(this, 'form.html', { userTriggeredOnInput: true }))
    ...
    it('should not record input values if dynamically added and maskAllInputs is true', async () => {
        const page: puppeteer.Page = await browser.newPage()
        await page.goto('about:blank')
        await page.setContent(getHtml.call(this, 'empty.html', { maskAllInputs: true }))
        ...
        await page.evaluate(() => {
            const ta = document.querySelector('textarea')
            ta.innerText = 'textarea attribute mutation via innerText should also be masked '
        })
    ...
    it('should record images with blob url', async () => {
        ...
        await page.goto(`${serverURL}/html`)
        page.setContent(getHtml.call(this, 'image-blob-url.html', { inlineImages: true }))
    ...
    it('should record images inside iframe with blob url', async () => {
        ...
        page.on('console', (msg) => console.log(msg.text()))
        await page.goto(`${serverURL}/html`)
        await page.setContent(getHtml.call(this, 'frame-image-blob-url.html', { inlineImages: true }))
        await page.waitForResponse(`${serverURL}/html/assets/robot.png`)
    ...
    it('should record images inside iframe with blob url after iframe was reloaded', async () => {
        ...
        page.on('console', (msg) => console.log(msg.text()))
        await page.goto(`${serverURL}/html`)
        await page.setContent(getHtml.call(this, 'frame2.html', { inlineImages: true }))
    ...
    it('should record shadow DOM', async () => {
        ...
        await page.evaluate(() => {
            ...
            sleep(1)
                ...
                .then(() => {
                    ;(shadowRoot.lastChild!.childNodes[0] as HTMLElement).innerText = '123'
                    const nestedShadowElement = shadowRoot.lastChild!.childNodes[0] as HTMLElement
                    nestedShadowElement.attachShadow({
                        mode: 'open',
                    })
                    nestedShadowElement.shadowRoot!.appendChild(document.createElement('span'))
                    ;(nestedShadowElement.shadowRoot!.lastChild as HTMLElement).innerText = 'nested shadow dom'
                })
        })
        await page.waitForTimeout(50)
    ...
    it('should keep recording shadow DOM mutations after an iframe is torn down', async () => {
      ...
      `) as Promise<number>
        ...
        await page.evaluate(`window.__log.appendChild(document.createElement('li'))`)
        await page.waitForTimeout(50)
    ...
    it('should record shadow DOM 2', async () => {
        ...
        await page.evaluate(() => {
            return new Promise((resolve) => {
                ...
                ;(el.shadowRoot as ShadowRoot).appendChild(document.createElement('input'))
                setTimeout(() => {
    ...
    it('should record nested iframes and shadow doms', async () => {
        ...
        await page.evaluate(() => {
            ...
            const contentDocument1 = document.querySelector('iframe')!.contentDocument!
            // create shadow dom #1
            contentDocument1.body.attachShadow({ mode: 'open' })
            contentDocument1.body.shadowRoot!.appendChild(document.createElement('div'))
        ...
        page.evaluate(() => {
            ...
            contentDocument2.body.shadowRoot!.appendChild(document.createElement('span'))
        })
      ...
      `
        ...
        await page.evaluate(
            async (OldColor, NewColor) => {
                ...
                const incrementalStyle = document.createElement('style') as HTMLStyleElement
                ...
                ;(styleElement.sheet!.cssRules[0] as any).style.setProperty('color', NewColor)
                // Change the color of the #two div element to yellow as an incremental style mutation
                ;(incrementalStyle.sheet!.cssRules[0] as any).style.setProperty('color', NewColor)
            },
            OldColor,
            NewColor
        )
