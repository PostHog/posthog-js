...
puppeteer.Page.prototype.screenshot = async function (...args: Parameters<typeof originalScreenshot>): Promise<Buffer> {
...
export async function launchPuppeteer(options?: Parameters<(typeof puppeteer)['launch']>[0]) {
    ...
    return JSON.stringify(
        snapshots
            ...
            .map((s) => {
                ...
                if (s.type === EventType.IncrementalSnapshot && s.data.source === IncrementalSource.MouseInteraction) {
                    delete (s.data as Optional<mouseInteractionData, 'x'>).x
                    delete (s.data as Optional<mouseInteractionData, 'y'>).y
                }
                if (s.type === EventType.IncrementalSnapshot && s.data.source === IncrementalSource.Mutation) {
                    ...
                    s.data.adds.forEach((add) => {
                        if (add.node.type === NodeType.Element) {
                            ...
                            // normalize rr_left/rr_top to avoid environment rendering differences
                            if ('rr_left' in add.node.attributes) add.node.attributes.rr_left = 'Npx'
                            if ('rr_top' in add.node.attributes) add.node.attributes.rr_top = 'Npx'
                        }
                    })
                ...
                } else if (s.type === EventType.Plugin && s.data.plugin === 'rrweb/console@1') {
                    const pluginPayload = (
                        s as pluginEvent<{
                            trace: string[]
                            payload: string[]
                    ...
                    if (pluginPayload?.trace.length) {
                        pluginPayload.trace = pluginPayload.trace.map((trace) => {
                            return trace.replace(/^pptr:.*?:(\d+:\d+)$/, '__puppeteer_evaluation_script__:$1')
                        })
                    }
                    if (pluginPayload?.payload.length) {
                        pluginPayload.payload = pluginPayload.payload.map((payload) => {
                            return payload.replace(/pptr:.*?:(\d+:\d+)/g, '__puppeteer_evaluation_script__:$1')
                        })
                    }
                }
...
function stripBlobURLsFromAttributes(node: {
    ...
    if (
        ...
        node.attributes.src = node.attributes.src.replace(/[\w-]+$/, '...').replace(/:[0-9]+\//, ':xxxx/')
    }
}
...
export async function assertSnapshot(snapshotsOrPage: eventWithTime[] | puppeteer.Page) {
    ...
    if (!Array.isArray(snapshotsOrPage)) {
        ...
        await snapshotsOrPage.waitForFunction('window.snapshots && window.snapshots.length > 0')

        snapshots = (await snapshotsOrPage.evaluate('window.snapshots')) as eventWithTime[]
    } else {
...
export function stripBase64(events: eventWithTime[]) {
    ...
    return events.map((evt) => {
        if (evt.type === EventType.IncrementalSnapshot && evt.data.source === IncrementalSource.CanvasMutation) {
...
export async function waitForRAF(pageOrFrame: puppeteer.Page | puppeteer.Frame) {
...
export async function waitForIFrameLoad(
    ...
    if (!el) throw new Error('Waiting for iframe load has timed out - no element found')

    let frame = await el.contentFrame()
    if (frame && frame.isDetached()) {
        throw new Error('Waiting for iframe load has timed out - frame is detached')
    }
    if (frame && frame.url() !== '') {
        return frame
    }
    ...
    await page.$eval(
        ...
        (el, timeout) => {
            const p = new Promise((resolve, reject) => {
                ...
                setTimeout(() => {
                    reject(new Error('Waiting for iframe load has timed out - onload not fired'))
                }, timeout)
            })
            return p
        },
    ...
    if (!frame) throw new Error('Waiting for iframe load has timed out - no frame found')
    return frame
}
