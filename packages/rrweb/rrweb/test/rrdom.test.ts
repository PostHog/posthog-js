...
import type { styleDeclarationData, styleSheetRuleData } from '@posthog/rrweb-types'
...
describe('diff algorithm for rrdom', () => {
  ...
  `)
    ...
    `)
            ...
            expect((styleEl.sheet?.cssRules[0] as CSSMediaRule).cssRules?.length).toEqual(3)
            expect((styleEl.sheet?.cssRules[0] as CSSMediaRule).cssRules[0].cssText).toEqual(cssText)
        })
    ...
    `)
            ...
            expect((styleEl.sheet?.cssRules[0] as CSSMediaRule).cssRules?.length).toEqual(1)
            expect((styleEl.sheet?.cssRules[0] as CSSMediaRule).cssRules[0].cssText).toEqual('a {color: blue;}')
        })
    })
})
