...
function toRecordPackPath(path) {
    return path.replace(/^([\w]+)\//, '$1/record/').replace('rrweb', 'rrweb-record-pack')
}

function toReplayPath(path) {
    return path.replace(/^([\w]+)\//, '$1/replay/').replace('rrweb', 'rrweb-replay')
}

function toReplayUnpackPath(path) {
    return path.replace(/^([\w]+)\//, '$1/replay/').replace('rrweb', 'rrweb-replay-unpack')
}
...
function toPluginPath(pluginName, stage) {
    return (path) => path.replace(/^([\w]+)\//, '$1/plugins/').replace('rrweb', `${pluginName}-${stage}`)
}
