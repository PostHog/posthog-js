...
import { isShadowRoot, IGNORED_NODE, classMatchesRegex } from '@posthog/rrweb-snapshot'
...
export function closestElementOfNode(node: Node | null): HTMLElement | null {
    ...
    const el: HTMLElement | null = node.nodeType === node.ELEMENT_NODE ? (node as HTMLElement) : dom.parentElement(node)
    return el
}
...
export function legacy_isTouchEvent(event: MouseEvent | TouchEvent | PointerEvent): event is TouchEvent {
...
export function isSerializedIframe<TNode extends Node | RRNode>(n: TNode, mirror: IMirror<TNode>): boolean {
    return Boolean(n.nodeName === 'IFRAME' && mirror.getMeta(n))
}

export function isSerializedStylesheet<TNode extends Node | RRNode>(n: TNode, mirror: IMirror<TNode>): boolean {
    return Boolean(
        n.nodeName === 'LINK' &&
...
export function getNestedRule(rules: CSSRuleList, position: number[]): CSSGroupingRule {
    const rule = rules[position[0]] as CSSGroupingRule
    if (position.length === 1) {
        return rule
    } else {
        return getNestedRule((rule.cssRules[position[1]] as CSSGroupingRule).cssRules, position.slice(2))
    }
}
