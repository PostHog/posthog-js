...
import type { mediaInteractionData, mediaAttributes } from '@posthog/rrweb-types'
...
export class MediaManager {
    private mediaMap: Map<HTMLMediaElement | RRMediaElement, MediaState> = new Map()
    ...
    private metadataCallbackMap: WeakMap<HTMLMediaElement | RRMediaElement, () => void> = new Map()
    ...
    private seekTo({
        ...
        if (mediaState.isPlaying) {
            ...
            const mediaPlaybackOffset =
                (differenceBetweenCurrentTimeAndMediaMutationTimestamp / 1000) * mediaState.playbackRate
    ...
    private getMediaStateFromMutation({
        ...
        const isPlaying =
            ...
            (type !== MediaInteractions.Pause && (lastState?.isPlaying || target.getAttribute('autoplay') !== null))

        const mediaState: MediaState = {
            isPlaying,
            currentTimeAtLastInteraction: currentTime ?? lastState?.currentTimeAtLastInteraction ?? 0,
    ...
    public addMediaElements(node: Node, timeOffset: number, mirror: Mirror) {
        ...
        const playerIsPaused = this.service.state.matches('paused')
        const mediaAttributes = serializedNode.attributes as mediaAttributes | Record<string, never>
