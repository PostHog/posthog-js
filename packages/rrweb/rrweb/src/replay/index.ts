...
import { RRDocument, createOrGetNode, buildFromNode, buildFromDom, diff, getDefaultSN } from '@posthog/rrdom'
import type {
    RRNode,
    RRElement,
    RRStyleElement,
...
export class Replayer {
    ...
    private constructedStyleMutations: (styleSheetRuleData | styleDeclarationData)[] = []
                ...
                if (Object.keys(this.legacy_missingNodeRetryMap).length) {
                    for (const key in this.legacy_missingNodeRetryMap) {
                        try {
                            ...
                            const realNode = createOrGetNode(value.node as RRNode, this.mirror, this.virtualDom.mirror)
                            diff(realNode, value.node as RRNode, replayerHandler, this.virtualDom.mirror)
                            value.node = realNode
                        } catch (error) {
                            this.warn(error)
                        }
     ...
     */
    private addTimeout(callback: () => void, delay: number): ReturnType<typeof setTimeout> {
    ...
    public setConfig(config: Partial<playerConfig>) {
        Object.keys(config).forEach((key) => {
            ...
            ;(this.config as Record<keyof playerConfig, typeof newConfigValue>)[key as keyof playerConfig] =
    ...
    public addEvent(rawEvent: eventWithTime | string) {
        const event = this.config.unpackFn ? this.config.unpackFn(rawEvent as string) : (rawEvent as eventWithTime)
    ...
    private setupDom() {
        ...
        if (this.iframe.contentWindow && this.iframe.contentDocument) {
            smoothscrollPolyfill(this.iframe.contentWindow, this.iframe.contentDocument)
    ...
    private applyFullscreen(payload: unknown) {
        ...
        if (!data || typeof data.id !== 'number' || typeof data.enter !== 'boolean') {
            return
        }
    ...
    private applyEvents = (events: Array<eventWithTime>, onApplied: () => void) => {
    ...
    private rebuildFullSnapshot(event: fullSnapshotEvent & { timestamp: number }, isSync = false) {
        if (!this.iframe.contentDocument) {
    ...
    private insertStyleRules(documentElement: HTMLElement | RRElement, head: HTMLHeadElement | RRElement) {
        ...
        if (this.usingVirtualDom) {
            ...
            ;(documentElement as HTMLElement).insertBefore(styleEl, head as HTMLHeadElement)
    ...
    private attachDocumentToIframe(mutation: addedNodeMutation, iframeEl: HTMLIFrameElement | RRIFrameElement) {
        const mirror: RRDOMMirror | Mirror = this.usingVirtualDom ? this.virtualDom.mirror : this.mirror
        ...
        const afterAppend = (builtNode: Node, id: number) => {
            ...
            if (sn?.type === NodeType.Element && sn?.tagName.toUpperCase() === 'HTML') {
                const { documentElement, head } = iframeEl.contentDocument!
                this.insertStyleRules(documentElement as HTMLElement | RRElement, head as HTMLElement | RRElement)
            }
    ...
    private collectIframeAndAttachDocument(collected: AppendedIframe[], builtNode: Node) {
    ...
    private async deserializeAndPreloadCanvasEvents(data: canvasMutationData, event: eventWithTime) {
            ...
            if ('commands' in data) {
                const commands = await Promise.all(
                    data.commands.map(async (c) => {
                        const args = await Promise.all(c.args.map(deserializeArg(this.imageMap, null, status)))
                        return { ...c, args }
                    })
                )
    ...
    private applyIncremental(e: incrementalSnapshotEvent & { timestamp: number; delay?: number }, isSync: boolean) {
        const { data: d } = e
        switch (d.source) {
            ...
            case IncrementalSource.StyleDeclaration: {
                if (this.usingVirtualDom) {
                    ...
                    else if (d.id) (this.virtualDom.mirror.getNode(d.id) as RRStyleElement | null)?.rules.push(d)
            ...
            case IncrementalSource.Font: {
                try {
                    const fontFace = new FontFace(
                        d.family,
                        d.buffer ? new Uint8Array(JSON.parse(d.fontSource) as Iterable<number>) : d.fontSource,
                        d.descriptors
                    )
    ...
    private applyMutation(d: mutationData, isSync: boolean) {
        ...
        if (this.config.useVirtualDom && !this.usingVirtualDom && isSync) {
            ...
            if (Object.keys(this.legacy_missingNodeRetryMap).length) {
                for (const key in this.legacy_missingNodeRetryMap) {
                    try {
                        const value = this.legacy_missingNodeRetryMap[key]
                        const virtualNode = buildFromNode(value.node as Node, this.virtualDom, this.mirror)
        ...
        validRemoves.forEach((mutation) => {
            ...
            let parent: Node | null | ShadowRoot | RRNode = mirror.getNode(mutation.parentId)
            if (!parent) {
        ...
        const appendNode = (mutation: addedNodeMutation) => {
            ...
            if (!parent) {
                ...
                const parentIdInCurrentBatch = d.adds.some((add) => add.node.id === mutation.parentId)
            ...
            if (isSerializedIframe<typeof parent>(parent, mirror)) {
                this.attachDocumentToIframe(mutation, parent as HTMLIFrameElement | RRIFrameElement)
                return
            }
            ...
            if (parentSn && parentSn.type === NodeType.Element && mutation.node.type === NodeType.Text) {
                ...
                if (parentSn.tagName === 'textarea') {
                    ...
                    for (const cssText of prospectiveSiblings as (Node & RRNode)[]) {
                        if (cssText.nodeType === parent.TEXT_NODE && !mirror.hasNode(cssText)) {
                ...
                if (target.nodeName === 'HTML' && parentDoc.documentElement)
                    parentDoc.removeChild(parentDoc.documentElement as HTMLElement & RRNode)
            }

            if (previous && previous.nextSibling && previous.nextSibling.parentNode) {
                ;(parent as TNode).insertBefore(target as TNode, previous.nextSibling as TNode)
            ...
            if (isSerializedIframe(target, this.mirror)) {
                ...
                if (mutationInQueue) {
                    this.attachDocumentToIframe(mutationInQueue, target as HTMLIFrameElement)
                    this.newDocumentQueue = this.newDocumentQueue.filter((m) => m !== mutationInQueue)
                }
            }
                    ...
                    if (value === null) {
                        ...
                        try {
                            ...
                            if (
                                ...
                                try {
                                    const newSn = mirror.getMeta(target as Node & RRNode) as serializedElementNodeWithId
                                    ...
                                    Object.assign(newSn.attributes, mutation.attributes as attributes)
                            ...
                            if (attributeName === 'value' && target.nodeName === 'TEXTAREA') {
                                ...
                                (mirror.getMeta(target as Node & RRNode) as serializedElementNodeWithId | null)?.isSVG
                            ) {
                                ...
                                ;(target as Element | RRElement).setAttribute(attributeName, value)
                            }
    ...
    private applyStyleSheetMutation(data: styleDeclarationData | styleSheetRuleData) {
        let styleSheet: CSSStyleSheet | null = null
        if (data.styleId) styleSheet = this.styleMirror.getStyle(data.styleId)
        else if (data.id) styleSheet = (this.mirror.getNode(data.id) as HTMLStyleElement)?.sheet || null
        if (!styleSheet) return
        if (data.source === IncrementalSource.StyleSheetRule) this.applyStyleSheetRule(data, styleSheet)
        else if (data.source === IncrementalSource.StyleDeclaration) this.applyStyleDeclaration(data, styleSheet)
    }

    private applyStyleSheetRule(data: styleSheetRuleData, styleSheet: CSSStyleSheet) {
        data.adds?.forEach(({ rule, index: nestedIndex }) => {
            try {
                if (Array.isArray(nestedIndex)) {
                    ...
                    const index =
                        nestedIndex === undefined ? undefined : Math.min(nestedIndex, styleSheet.cssRules.length)
    ...
    private applyStyleDeclaration(data: styleDeclarationData, styleSheet: CSSStyleSheet) {
        if (data.set) {
            const rule = getNestedRule(styleSheet.rules, data.index) as unknown as CSSStyleRule
    ...
    private applyAdoptedStyleSheet(data: adoptedStyleSheetData) {
        ...
        data.styles?.forEach((style) => {
            ...
            if (hasShadowRoot(targetHost)) hostWindow = targetHost.ownerDocument?.defaultView || null
            else if (targetHost.nodeName === '#document') hostWindow = (targetHost as Document).defaultView
        ...
        const adoptStyleSheets = (targetHost: Node, styleIds: number[]) => {
            ...
            else if (targetHost.nodeName === '#document') (targetHost as Document).adoptedStyleSheets = stylesToAdopt
    ...
    private legacy_resolveMissingNode(
        ...
        if (nextInMap) {
            ...
            parent.insertBefore(node as Node & RRNode, target.nextSibling as Node & RRNode)
            delete map[mutation.node.id]
            delete this.legacy_missingNodeRetryMap[mutation.node.id]
            if (mutation.previousId || mutation.nextId) {
                this.legacy_resolveMissingNode(map, parent, node, mutation)
    ...
    private warnCanvasMutationFailed(d: canvasMutationData | canvasMutationCommand, error: unknown) {
