...
import type { eventWithTime, eventWithoutTime, serializedNodeWithId, mutationCallBack } from '@posthog/rrweb-types'
...
export class IframeManager {
    ...
    private nestedIframeListeners: Map<Window, (message: MessageEvent) => void> = new Map()
    // Originals captured per iframe so cleanup survives iframe.src swaps.
    private attachedWindows: WeakMap<HTMLIFrameElement, Set<Window>> = new WeakMap()
    private attachedDocuments: WeakMap<HTMLIFrameElement, Set<Document>> = new WeakMap()
    private attachedIframes: Map<number, { element: HTMLIFrameElement; content: serializedNodeWithId }> = new Map()
    // Set per element — one iframe collects multiple disposers across loads.
    private loadListenerDisposers: WeakMap<HTMLIFrameElement, Set<() => void>> = new WeakMap()
    ...
    private pageHideHandlers: WeakMap<HTMLIFrameElement, Set<{ win: Window; handler: () => void }>> = new WeakMap()
    ...
    public registerLoadListenerDisposer(iframeEl: HTMLIFrameElement, disposer: () => void) {
    ...
    public getIframeElementById(iframeId: number): HTMLIFrameElement | null {
        return this.attachedIframes.get(iframeId)?.element ?? this.iframeElementsById.get(iframeId) ?? null
    }
    ...
    private trackIframeContent(iframeEl: HTMLIFrameElement, content: serializedNodeWithId): number {
    ...
    private transformCrossOriginEvent(iframeEl: HTMLIFrameElement, e: eventWithTime): eventWithTime | false {
        switch (e.type) {
