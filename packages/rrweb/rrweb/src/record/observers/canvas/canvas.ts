...
import type { blockClass, IWindow, listenerHandler } from '@posthog/rrweb-types'
...
function getRequiredWebGPUTextureUsage(win: IWindow) {
    const textureUsage = (win as IWindow & { GPUTextureUsage?: GPUTextureUsageLike }).GPUTextureUsage
...
function getCanvasFromWebGPUContext(context: WebGPUCanvasContextLike): WebGPUCanvasLike | null {
...
function isCanvasNode(canvas: WebGPUCanvasLike): canvas is ICanvas | HTMLCanvasElement {
...
function initCanvasWebGPUContextObserver(
    ...
    if (!GPUCanvasContext?.prototype || typeof GPUCanvasContext.prototype.configure !== 'function') {
        return
    }
    ...
    return patch(
        ...
        'configure',
        function (original: (this: WebGPUCanvasContextLike, configuration: GPUCanvasConfigurationLike) => void) {
            return function (this: WebGPUCanvasContextLike, configuration: GPUCanvasConfigurationLike) {
                ...
                return original.call(this, {
                    ...
                    usage:
                        typeof configuration.usage === 'number' ? configuration.usage | requiredUsage : requiredUsage,
                })
            }
        }
    )
...
export default function initCanvasContextObserver(
    ...
    try {
        ...
        const restoreHandler = patch(
            ...
            'getContext',
            function (
                original: (this: ICanvas | HTMLCanvasElement, contextType: string, ...args: Array<unknown>) => void
            ) {
                return function (this: ICanvas | HTMLCanvasElement, contextType: string, ...args: Array<unknown>) {
