...
type pendingCanvasMutationsMap = Map<HTMLCanvasElement, canvasMutationWithType[]>
...
export class CanvasManager {
    ...
    constructor(options: {
        ...
        if (recordCanvas && sampling === 'all')
            this.initCanvasMutationObserver(win, blockClass, blockSelector, dataURLOptions)
    ...
    private processMutation: canvasManagerMutationCallback = (target, mutation) => {
    ...
    private initCanvasFPSObserver(
        ...
        const scale =
            typeof options.resolutionScale === 'number' && Number.isFinite(options.resolutionScale)
                ? Math.min(1, Math.max(MIN_CANVAS_RESOLUTION_SCALE, options.resolutionScale))
                : 1
    ...
    flushPendingCanvasMutations() {
        this.pendingCanvasMutations.forEach((_values: canvasMutationCommand[], canvas: HTMLCanvasElement) => {
            const id = this.mirror.getId(canvas)
