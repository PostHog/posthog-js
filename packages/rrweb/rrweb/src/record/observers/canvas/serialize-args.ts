...
export const saveWebGLVar = (value: unknown, win: IWindow, ctx: RenderingContext): number | void => {
...
export function serializeArg(
    ...
    if (value instanceof Array) {
        ...
        return {
            rr_type: name,
            args: [serializeArg(value.buffer, win, ctx, dataURLOptions), value.byteOffset, value.byteLength],
        }
        ...
        return {
            rr_type: name,
            args: [serializeArg(value.data, win, ctx, dataURLOptions), value.width, value.height],
        }
...
export const isInstanceOfWebGLObject = (
    ...
    return Boolean(supportedWebGLConstructorNames.find((name: string) => value instanceof win[name as keyof Window]))
}
