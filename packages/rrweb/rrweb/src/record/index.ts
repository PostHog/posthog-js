...
function inlineDeferredStylesheets(
    ...
    const step = (deadline?: IdleDeadline) => {
        ...
        do {
            ...
            index += 1
            if (link) {
                callSafely(() => stylesheetManager.inlineDeferredLinkElement(link, mirror.getId(link)))
            }
        } while (index < links.length && deadline && !deadline.didTimeout && deadline.timeRemaining() > 5)
...
function record<T = eventWithTime>(options: recordOptions<T> = {}): listenerHandler | undefined {
    ...
    const slimDOMOptions = slimDOMDefaults(_slimDOMOptions !== undefined ? _slimDOMOptions : false)
    ...
    wrappedEmit = (r: eventWithoutTime, isCheckout?: boolean) => {
        ...
        if (
            ...
            !(e.type === EventType.IncrementalSnapshot && nonUserInitiatedSources.has(e.data.source))
        ) {
    ...
    takeFullSnapshot = (isCheckout = false) => {
        ...
        try {
            node = snapshot(document, {
                ...
                onIframeListenerRegistered: (iframe: HTMLIFrameElement, disposer: () => void) => {
