import { snapshot, type MaskInputOptions, type SlimDOMOptions, createMirror } from '@posthog/rrweb-snapshot'
...
// Declared at module level, populated per recording session, cleared on destroy
let iframeObserverCleanups: Map<number, listenerHandler> = new Map()

function cleanupDetachedIframeObservers(mirror: ReturnType<typeof createMirror>) {
...
function record<T = eventWithTime>(options: recordOptions<T> = {}): listenerHandler | undefined {
    ...
    const wrappedMutationEmit = (m: mutationCallbackParam) => {
        ...
        if (recordCrossOriginIframes && m.removes && m.removes.length > 0) {
            ...
            cleanupDetachedIframeObservers(mirror)
        }
