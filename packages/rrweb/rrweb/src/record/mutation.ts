...
function getSerializedAttributeName(target: Element, localName: string, namespace: string | null): string {
...
export default class MutationBuffer {
    ...
    private processBufferedMutations = () => {
        ...
        for (const n of this.movedSet) {
            if (isParentRemoved(this.removesSubTreeCache, n, this.mirror) && !this.movedSet.has(dom.parentNode(n)!)) {
                continue
            }
            pushAdd(n)
        }
                        ...
                        for (const [name, value] of Object.entries(attributes)) {
                            if (typeof value === 'string' || value === null) {
                                attributes[name] = maskAttributeValue({
                                    ...
                                    maskAllElementAttributes: this.maskAllElementAttributes,
                                    maskAttributeFn: this.maskAttributeFn,
                                    isGenerated: this.generatedAttributes.get(attribute.node)?.has(name),
                                })
                            }
                        }
                    }
    ...
    private processMutation = (m: mutationRecord) => {
        ...
        switch (m.type) {
            ...
            case 'attributes': {
                ...
                const isIframeSrc = target.tagName === 'IFRAME' && attributeName === 'src'
                if (
                    isIframeSrc &&
