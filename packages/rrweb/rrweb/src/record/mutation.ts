...
export default class MutationBuffer {
    ...
    private genTextAreaValueMutation = (textarea: HTMLTextAreaElement) => {
        ...
        item.attributes.value = Array.from(dom.childNodes(textarea), (cn) => dom.textContent(cn) || '').join('')
    }
    ...
    private processMutation = (m: mutationRecord) => {
        ...
        switch (m.type) {
            ...
            case 'attributes': {
                ...
                if (target.tagName === 'IFRAME' && attributeName === 'src' && !this.keepIframeSrcFn(value as string)) {
