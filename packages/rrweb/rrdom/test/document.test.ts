...
describe('Basic RRDocument implementation', () => {
    ...
    describe('Basic RRNode implementation', () => {
        ...
        it('can get childNodes', () => {
            ...
            expect(parentNode.childNodes).toEqual([childNode1, childNode2, childNode3])
        })
        ...
        it('should return whether the node contains another node', () => {
            ...
            expect(parentNode.contains(new RRDocument().createElement('div'))).toBeFalsy()
            const childNode1 = new RRNode()
            const childNode2 = new RRNode()
            parentNode.firstChild = childNode1
            parentNode.lastChild = childNode1
    ...
    describe('Basic RRDocument implementation', () => {
        ...
        it('can remove an existing child', () => {
            ...
            expect(() => node.removeChild(node.createElement('div'))).toThrowErrorMatchingInlineSnapshot(
        ...
        it('should implement create node functions', () => {
            const node = new RRDocument()
            expect(node.createDocument(null, '', null).RRNodeType).toEqual(RRNodeType.Document)
            expect(node.createDocumentType('', '', '').RRNodeType).toEqual(RRNodeType.DocumentType)
            expect(node.createElement('html').RRNodeType).toEqual(RRNodeType.Element)
            expect(node.createElementNS('', 'html').RRNodeType).toEqual(RRNodeType.Element)
            expect(node.createTextNode('text').RRNodeType).toEqual(RRNodeType.Text)
            expect(node.createComment('comment').RRNodeType).toEqual(RRNodeType.Comment)
            expect(node.createCDATASection('data').RRNodeType).toEqual(RRNodeType.CDATA)
        })
        ...
        it('can cover the usage of write() in rrweb-snapshot', () => {
            const node = new RRDocument()
            node.write('<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "">')
            ...
            expect(doctype.publicId).toEqual('-//W3C//DTD XHTML 1.0 Transitional//EN')
            expect(doctype.systemId).toEqual('')

            node.write('<!DOCTYPE html PUBLIC "-//W3C//DTD HTML 4.0 Transitional//EN" "">')
            expect(node.childNodes.length).toBe(1)
    ...
    describe('Basic RRElement implementation', () => {
        ...
        it('can remove CSS property', () => {
            ...
            expect(node.attributes.style).toEqual('background-color: red; width: 78%; height: 50vh !important;')
      ...
      `
        ...
        it('can remove an existing child', () => {
            ...
            expect(() => node.removeChild(document.createElement('div'))).toThrowErrorMatchingInlineSnapshot(
                `[Error: Failed to execute 'removeChild' on 'RRNode': The RRNode to be removed is not a child of this RRNode.]`
            )
