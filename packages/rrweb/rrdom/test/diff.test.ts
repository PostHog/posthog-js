...
import { buildFromDom, getDefaultSN, Mirror as RRNodeMirror, RRDocument, RRMediaElement, printRRDom } from '../src'
import { createOrGetNode, diff, ReplayerHandler, nodeMatching, sameNodeType } from '../src/diff'
import type { IRRElement, IRRNode } from '../src/document'
import type { serializedNodeWithId, canvasMutationData, styleSheetRuleData } from '@posthog/rrweb-types'
import { NodeType as RRNodeType, EventType, IncrementalSource } from '@posthog/rrweb-types'
...
describe('diff algorithm for rrdom', () => {
    ...
    describe('diff single node', () => {
        it('should diff a document node', () => {
            ...
            expect(document.doctype?.publicId).toEqual('-//W3C//DTD XHTML 1.0 Transitional//EN')
        ...
        it('should diff a style element', () => {
            ...
            expect(replayer.applyStyleSheetMutation).toHaveBeenCalledWith(styleData, element.sheet)
        })
        ...
        it('should diff a media element', async () => {
            ...
            for (const tagName of ['AUDIO', 'VIDEO']) {
                ...
                const rrDocument = new RRDocument()
                const rrMedia = rrDocument.createElement(tagName) as unknown as RRMediaElement
                rrMedia.volume = 0.5
    ...
    describe('diff properties', () => {
        ...
        it('can update exist properties', () => {
            ...
            expect((node as Node as HTMLElement).id).toBe('node1')
            expect((node as Node as HTMLElement).className).toBe('node')
            expect((node as Node as HTMLElement).getAttribute('style')).toBe('color: white')
    ...
    describe('diff children', () => {
        it('append elements', () => {
            ...
            expect(rrNode.childNodes.length).toEqual(3)
            expect(Array.from(node.childNodes).map((c) => mirror.getId(c))).toEqual([1, 2, 3])
        })
        ...
        it('prepends elements', () => {
            ...
            expect(rrNode.childNodes.length).toEqual(5)
            expect(Array.from(node.childNodes).map((c) => mirror.getId(c))).toEqual([1, 2, 3, 4, 5])
        })
        ...
        it('remove elements from the beginning', () => {
            ...
            expect(node.childNodes.length).toEqual(3)
            expect(rrNode.childNodes.length).toEqual(3)
            expect(Array.from(node.childNodes).map((c) => mirror.getId(c))).toEqual([3, 4, 5])
        })
        ...
        it('remove elements from the middle', () => {
            ...
            expect(node.childNodes.length).toEqual(4)
            expect(rrNode.childNodes.length).toEqual(4)
            expect(Array.from(node.childNodes).map((c) => mirror.getId(c))).toEqual([1, 2, 4, 5])
        })
        ...
        it('moves element forward', () => {
            ...
            expect(node.childNodes.length).toEqual(5)
            expect(rrNode.childNodes.length).toEqual(5)
            expect(Array.from(node.childNodes).map((c) => mirror.getId(c))).toEqual([2, 3, 4, 1, 5])
        })
        ...
        it('move elements to end', () => {
            ...
            expect(node.childNodes.length).toEqual(3)
            expect(rrNode.childNodes.length).toEqual(3)
            expect(Array.from(node.childNodes).map((c) => mirror.getId(c))).toEqual([2, 3, 1])
        })
        ...
        it('move element backwards', () => {
            ...
            expect(node.childNodes.length).toEqual(4)
            expect(rrNode.childNodes.length).toEqual(4)
            expect(Array.from(node.childNodes).map((c) => mirror.getId(c))).toEqual([1, 4, 2, 3])
        })
        ...
        it('swap first and last', () => {
            ...
            expect(node.childNodes.length).toEqual(4)
            expect(rrNode.childNodes.length).toEqual(4)
            expect(Array.from(node.childNodes).map((c) => mirror.getId(c))).toEqual([4, 2, 3, 1])
        })
        ...
        it('move to left and replace', () => {
            ...
            expect(node.childNodes.length).toEqual(5)
            expect(rrNode.childNodes.length).toEqual(5)
            expect(Array.from(node.childNodes).map((c) => mirror.getId(c))).toEqual([4, 1, 2, 3, 6])
        })
        ...
        it('move to left and leaves hold', () => {
            ...
            expect(node.childNodes.length).toEqual(2)
            expect(rrNode.childNodes.length).toEqual(2)
            expect(Array.from(node.childNodes).map((c) => mirror.getId(c))).toEqual([4, 6])
        })
        ...
        it('reverse elements', () => {
            ...
            expect(node.childNodes.length).toEqual(8)
            expect(rrNode.childNodes.length).toEqual(8)
            expect(Array.from(node.childNodes).map((c) => mirror.getId(c))).toEqual([8, 7, 6, 5, 4, 3, 2, 1])
        })
        ...
        it('handle random shuffle 1', () => {
            ...
            expect(rrNode.childNodes.length).toEqual(newElementsNum)
            expect(Array.from(node.childNodes).map((c) => mirror.getId(c))).toEqual(newElementsIds)
        })
    ...
    describe('diff shadow dom', () => {
        it('should add a shadow dom', () => {
            ...
            expect((node as Node as HTMLElement).shadowRoot!.childNodes.length).toBe(1)
            const childElement = (node as Node as HTMLElement).shadowRoot!.childNodes[0] as HTMLElement
    ...
    describe('diff iframe elements', () => {
        ...
        it('should add an element to the contentDocument of an iframe element', () => {
            ...
            rrDocument.mirror.add(rrNode.contentDocument, getDefaultSN(rrNode.contentDocument, 1))
        ...
        it('should remove children from document before adding new nodes 2', () => {
            ...
            iframe.contentDocument!.write('<!DOCTYPE html PUBLIC "-//W3C//DTD HTML 4.0 Transitional//EN" "">')
        ...
        it('should remove children from document before adding new nodes 3', () => {
            ...
            rrDocument.mirror.add(rrIframeEl.contentDocument!, getDefaultSN(rrIframeEl.contentDocument!, 4))
        ...
        it('should remove children from document before adding new nodes 4', () => {
            ...
            ;[-2, -3, -4, -5, -6].forEach((id) => expect(mirror.getNode(id)).toBeNull())
        ...
        it('selectors should be case-sensitive for matching in iframe dom', async () => {
            ...
            try {
                const code = fs.readFileSync(path.resolve(__dirname, '../dist/rrdom.umd.cjs'), 'utf8')
                ...
                const caseInsensitiveInNormalDom = await page.evaluate((className) => {
                    document.write('<!DOCTYPE html><html><body><iframe></iframe></body></html>')
                ...
                const caseInsensitiveInDefaultIFrameDom = await page.evaluate((className) => {
    ...
    describe('afterAppend callback', () => {
        ...
        it('should call afterAppend callback in the post traversal order', () => {
            ...
            ;[2, 5, 6, 4, 7, 9, 10, 8, 3, 1].forEach((id, index) => {
                ...
                expect(afterAppendFn).toHaveBeenNthCalledWith(index + 1, mirror.getNode(id), id)
            })
        })
    ...
    describe('create or get a Node', () => {
        ...
        it('create a node from RRNode', () => {
            ...
            expect(() => createOrGetNode(rrNode, mirror, rrDocument.mirror)).toThrowErrorMatchingInlineSnapshot(
                `DOMException {}`
            )
        })
