...
import type { IRRCDATASection, IRRComment, IRRDocumentType, IRRElement, IRRNode, IRRText } from './document'
import type {
    RRCanvasElement,
    RRDialogElement,
    RRElement,
...
export function diff(
    ...
    rrnodeMirror: Mirror = (newTree as RRDocument).mirror || (newTree.ownerDocument as RRDocument).mirror
) {
 ...
 */
function diffBeforeUpdatingChildren(oldTree: Node, newTree: IRRNode, replayer: ReplayerHandler, rrnodeMirror: Mirror) {
        ...
        case RRNodeType.Element: {
            ...
            switch (newRRElement.tagName) {
                case 'IFRAME': {
                    ...
                    diff(oldContentDocument, (newTree as RRIFrameElement).contentDocument, replayer, rrnodeMirror)
                    break
                }
            }
 ...
 */
function diffAfterUpdatingChildren(oldTree: Node, newTree: IRRNode, replayer: ReplayerHandler) {
    switch (newTree.RRNodeType) {
        case RRNodeType.Document: {
        ...
        case RRNodeType.Element: {
            ...
            switch (newRRElement.tagName) {
                ...
                case 'VIDEO': {
                    ...
                    if (newMediaRRElement.loop !== undefined) oldMediaElement.loop = newMediaRRElement.loop
                    break
                }
                case 'CANVAS': {
        ...
        case RRNodeType.CDATA: {
            if (oldTree.textContent !== (newTree as IRRText | IRRComment | IRRCDATASection).data)
                oldTree.textContent = (newTree as IRRText | IRRComment | IRRCDATASection).data
            break
        }
    }
...
function diffProps(oldTree: HTMLElement, newTree: IRRElement, rrnodeMirror: Mirror) {
    ...
    for (const name in newAttributes) {
        ...
        if (sn?.isSVG && NAMESPACES[name]) oldTree.setAttributeNS(NAMESPACES[name], name, newValue)
...
function diffChildren(oldTree: Node, newTree: IRRNode, replayer: ReplayerHandler, rrnodeMirror: Mirror) {
...
export function createOrGetNode(rrNode: IRRNode, domMirror: NodeMirror, rrnodeMirror: Mirror): Node {
...
export function sameNodeType(node1: Node, node2: IRRNode) {
    ...
    return (
        ...
        (node1 as HTMLElement).tagName.toUpperCase() === (node2 as IRRElement).tagName
    )
}
 ...
 */
export function nodeMatching(node1: Node, node2: IRRNode, domMirror: NodeMirror, rrdomMirror: Mirror): boolean {
