    ...
    switch (node.nodeType) {
        case NodeType.DOCUMENT_NODE:
            if (parentRRNode && parentRRNode.nodeName === 'IFRAME')
                ...
                rrNode = rrdom
                ;(rrNode as IRRDocument).compatMode = (node as Document).compatMode as 'BackCompat' | 'CSS1Compat'
            }
            break
...
export function buildFromDom(
    ...
    function walk(node: Node, parentRRNode: IRRNode | null) {
        ...
        if (node.nodeName === 'IFRAME') {
            ...
            if (node.nodeType === NodeType.ELEMENT_NODE && (node as HTMLElement).shadowRoot)
...
function walk(node: IRRNode, mirror: IMirror<IRRNode>, blankSpace: string) {
    ...
    if (node.RRNodeType === RRNodeType.Element) {
        ...
        if (element.shadowRoot) printText += walk(element.shadowRoot, mirror, blankSpace + '  ')
    }
    for (const child of node.childNodes) printText += walk(child, mirror, blankSpace + '  ')
    if (node.nodeName === 'IFRAME')
        printText += walk((node as RRIFrameElement).contentDocument, mirror, blankSpace + '  ')
    return printText
}
