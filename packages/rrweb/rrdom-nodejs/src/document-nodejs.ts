...
export class RRDocument extends BaseRRDocument implements IRRDocument {
    ...
    get nwsapi(): NWSAPI {
        if (!this._nwsapi) {
            ...
            this._nwsapi = nwsapi({
                ...
                DOMException: null as unknown as new (message?: string, name?: string) => DOMException,
            }) as NWSAPI
    ...
    getElementsByTagName(tagName: string): RRElement[] {
        if (this.documentElement) return this.documentElement.getElementsByTagName(tagName)
        return []
    }

    getElementsByClassName(className: string): RRElement[] {
        if (this.documentElement) return this.documentElement.getElementsByClassName(className)
        return []
    }

    getElementById(elementId: string): RRElement | null {
        if (this.documentElement) return this.documentElement.getElementById(elementId)
        return null
    }
...
export class RRElement extends BaseRRElement {
    ...
    get firstElementChild(): RRElement | null {
        for (const child of this.childNodes) if (child.RRNodeType === RRNodeType.Element) return child as RRElement
        return null
    }
    ...
    querySelectorAll(selectors: string): BaseRRNode[] {
        ...
        if (this.ownerDocument !== null) {
            ;(this.ownerDocument as RRDocument).nwsapi.select(selectors, this as unknown as Element, (element) => {
                if ((element as unknown as RRElement) !== this) result.push(element as unknown as RRElement)
    ...
    getElementsByClassName(className: string): RRElement[] {
        ...
        if (
            ...
            elements.push(this)
        for (const child of this.childNodes) {
            if (child instanceof RRElement) elements = elements.concat(child.getElementsByClassName(className))
        }
        return elements
    }
    ...
    getElementsByTagName(tagName: string): RRElement[] {
        ...
        if (this instanceof RRElement && this.tagName === normalizedTagName) elements.push(this)
        for (const child of this.childNodes) {
            if (child instanceof RRElement) elements = elements.concat(child.getElementsByTagName(tagName))
        }
        return elements
    }
}
...
export class RRStyleElement extends RRElement {
    ...
    get sheet() {
        if (!this._sheet) {
            let result = ''
            for (const child of this.childNodes)
                if (child.RRNodeType === RRNodeType.Text) result += (child as RRText).textContent
...
type RRElementType<K extends keyof HTMLElementTagNameMap> = K extends keyof RRElementTagNameMap
    ? RRElementTagNameMap[K]
    : RRElement
