...
export function polyfillRAF() {
    if (typeof window !== 'undefined' || 'requestAnimationFrame' in global) return
    ...
    function requestAnimationFrame(callback: (timestamp: number) => void) {
        ...
        if (timeoutHandle === null) timeoutHandle = setTimeout(onFrameTimer, INTERVAL)
        return cbHandle
    }
    ...
    ;(global as Window & typeof globalThis).requestAnimationFrame = requestAnimationFrame
    ;(global as Window & typeof globalThis).cancelAnimationFrame = cancelAnimationFrame
}
