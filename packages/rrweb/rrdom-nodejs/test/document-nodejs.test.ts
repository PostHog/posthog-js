...
import { buildFromDom } from '@posthog/rrdom'
...
describe('RRDocument for nodejs environment', () => {
    describe('RRDocument API', () => {
        ...
        it('can create different type of RRNodes', () => {
            ...
            const elementNS = rrdom.createElementNS('http://www.w3.org/2000/svg', 'div')
        ...
        it('can insert elements', () => {
            ...
            expect(() => rrdom.insertBefore(rrdom.createElement('div'), null)).toThrowErrorMatchingInlineSnapshot(
        ...
        it('getElementsByTagName', () => {
            for (let tagname of [
                ...
                expect(rrdom.getElementsByTagName(tagname).length).toEqual(expectedResult)
                expect(rrdom.getElementsByTagName(tagname.toLowerCase()).length).toEqual(expectedResult)
        ...
        it('getElementById', () => {
            ...
            for (let elementId of ['block', 'blocks', 'blocks1']) expect(rrdom.getElementById(elementId)).toBeNull()
            const node = new RRDocument()
        ...
        it('querySelectorAll querying tag name', () => {
            ...
            expect((rrdom.querySelectorAll('H1')[0] as RRElement).tagName).toEqual('H1')
            expect(rrdom.querySelectorAll('H1')[1]).toBeInstanceOf(RRElement)
            expect((rrdom.querySelectorAll('H1')[1] as RRElement).tagName).toEqual('H1')
        })
        ...
        it('querySelectorAll', () => {
            expect(rrdom.querySelectorAll('link[rel="stylesheet"]').length).toEqual(1)
            const targetLink = rrdom.querySelectorAll('link[rel="stylesheet"]')[0] as RRElement
    ...
    describe('RRElement API', () => {
        ...
        it('can get attribute', () => {
            expect(rrdom.getElementsByTagName('DIV')[0].getAttribute('class')).toEqual('blocks blocks1')
            expect(rrdom.getElementsByTagName('dIv')[0].getAttribute('cLaSs')).toEqual('blocks blocks1')
            expect(rrdom.getElementsByTagName('DIV')[0].getAttribute('id')).toEqual('block1')
            expect(rrdom.getElementsByTagName('div')[0].getAttribute('iD')).toEqual('block1')
            expect(rrdom.getElementsByTagName('p')[0].getAttribute('class')).toBeNull()
        })
        ...
        it('get nextElementSibling', () => {
            ...
            expect(rrdom.documentElement!.firstElementChild!.nextElementSibling).not.toBeNull()
            expect(rrdom.documentElement!.firstElementChild!.nextElementSibling!.tagName).toEqual('BODY')
            expect(rrdom.documentElement!.firstElementChild!.nextElementSibling!.nextElementSibling).toBeNull()
        ...
        it('can get CSS style declaration', () => {
            ...
            node.attributes.style = 'color: blue; background-color: red; width: 78%; height: 50vh !important;'
        ...
        it('can set CSS property', () => {
            ...
            expect(node.attributes.style).toEqual('color: red; height: 50vh !important;')

            // kebab-case
            style.setProperty('background-color', 'red')
            expect(node.attributes.style).toEqual('color: red; height: 50vh !important; background-color: red;')
        ...
        it('can remove CSS property', () => {
            const node = rrdom.createElement('div')
            node.attributes.style = 'color: blue; background-color: red; width: 78%; height: 50vh;'
            const style = node.style
            expect(style.removeProperty('color')).toEqual('blue')
            expect(node.attributes.style).toEqual('background-color: red; width: 78%; height: 50vh;')
        ...
        it('can parse more inline styles correctly', () => {
            ...
            expect(style.fontFamily).toEqual('"Goudy Bookletter 1911", Gill Sans Extrabold, sans-serif')
            ...
            // url
            node.attributes.style = 'background-image: url("http://example.com/img.png")'
            expect(node.style.backgroundImage).toEqual('url(http://example.com/img.png)')

            // comment
            node.attributes.style = 'top: 0; /* comment1 */ bottom: /* comment2 */42rem;'
        ...
        it('can insert new child before an existing child', () => {
            ...
            expect(() => node.insertBefore(node, child1)).toThrowErrorMatchingInlineSnapshot(
        ...
        it('style element', () => {
            ...
            const styleElement = rrdom.getElementsByTagName('style')[0] as RRStyleElement
            ...
            expect(rules[3].cssText).toEqual(`#block1 {width: 100px; height: 200px;}`)
