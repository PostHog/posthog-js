...
import { polyfillPerformance, polyfillRAF, polyfillEvent, polyfillNode, polyfillDocument } from '../src/polyfill'
...
describe('polyfill for nodejs', () => {
    it('should polyfill performance api', () => {
        if (compare(process.version, 'v16.0.0', '<')) expect(global.performance).toBeUndefined()
        polyfillPerformance()
    ...
    it('should not polyfill requestAnimationFrame if it already exists', () => {
        const fakeRequestAnimationFrame = vi.fn() as unknown as typeof global.requestAnimationFrame
        global.requestAnimationFrame = fakeRequestAnimationFrame
        const fakeCancelAnimationFrame = vi.fn() as unknown as typeof global.cancelAnimationFrame
    ...
    it('should polyfill Event type', () => {
        ...
        if (compare(process.version, 'v15.0.0', '<')) expect(global.Event).toBeUndefined()
        polyfillEvent()
