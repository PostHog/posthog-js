...
async function waitForFile(filePath: string, timeoutMs = 5000, intervalMs = 100): Promise<boolean> {
...
function minifyAndUMDPlugin({ name, outDir }: { name: LibraryOptions['name']; outDir: string }): Plugin {
    return {
        name: 'minify-plugin',
        async writeBundle(outputOptions, bundle) {
            for (const file of Object.values(bundle)) {
                if (file.type === 'asset' && (file.fileName.endsWith('.cjs.map') || file.fileName.endsWith('.css'))) {
                    ...
                    const fileReady = await waitForFile(inputFilePath)
                    if (!fileReady) {
                        console.warn(`Skipping minification: ${inputFilePath} not found after timeout`)
                        continue
                    }
                    if (isCSS) {
                        await buildFile({
...
export default function (
    ...
    return defineConfig(() => ({
        ...
        plugins: [
            ...
            visualizer({
                filename: resolve(process.cwd(), name + '-bundle-analysis.html'), // Path for the HTML report
