...
import type { serializedNode, serializedNodeWithId } from '@posthog/rrweb-types'
...
describe('utils', () => {
    describe('isNodeMetaEqual()', () => {
        ...
        it('should return false if two nodes have different node types', () => {
            expect(
                isNodeMetaEqual(undefined as unknown as serializedNode, null as unknown as serializedNode)
            ).toBeFalsy()
        ...
        it('should compare meta data of two document nodes', () => {
            expect(isNodeMetaEqual(document1, JSON.parse(JSON.stringify(document1)))).toBeTruthy()
            expect(isNodeMetaEqual(JSON.parse(JSON.stringify(document2)), document2)).toBeTruthy()
            expect(isNodeMetaEqual(document1, document2)).toBeFalsy()
        })

        it('should compare meta data of two documentType nodes', () => {
            expect(isNodeMetaEqual(documentType1, JSON.parse(JSON.stringify(documentType1)))).toBeTruthy()
            expect(isNodeMetaEqual(JSON.parse(JSON.stringify(documentType2)), documentType2)).toBeTruthy()
            expect(isNodeMetaEqual(documentType1, documentType2)).toBeFalsy()
        })

        it('should compare meta data of two text nodes', () => {
            expect(isNodeMetaEqual(text1, JSON.parse(JSON.stringify(text1)))).toBeTruthy()
            expect(isNodeMetaEqual(JSON.parse(JSON.stringify(text2)), text2)).toBeTruthy()
            expect(isNodeMetaEqual(text1, text2)).toBeFalsy()
        })

        it('should compare meta data of two comment nodes', () => {
            expect(isNodeMetaEqual(comment1, JSON.parse(JSON.stringify(comment1)))).toBeTruthy()
            expect(isNodeMetaEqual(JSON.parse(JSON.stringify(comment2)), comment2)).toBeTruthy()
            expect(isNodeMetaEqual(comment1, comment2)).toBeFalsy()
        })

        it('should compare meta data of two HTML elements', () => {
            expect(isNodeMetaEqual(element1, JSON.parse(JSON.stringify(element1)))).toBeTruthy()
            expect(isNodeMetaEqual(JSON.parse(JSON.stringify(element2)), element2)).toBeTruthy()
            expect(
    ...
    describe('escapeImportStatement', () => {
        it('parses imports with quotes correctly', () => {
            ...
            expect(out2).toEqual(`@import url("/foo.css;900;800\\"") supports(display: flex);`)
    ...
    describe('stringifyStylesheet', () => {
        ...
        `,
            ...
            expect(stringifyStylesheet(mockSheet)?.replace(/\s+/g, ' ').trim()).toEqual(
