...
describe('integration tests', function (this: ISuite) {
    ...
    beforeAll(async () => {
        ...
        code = fs.readFileSync(path.resolve(__dirname, '../dist/rrweb-snapshot.umd.cjs'), 'utf-8')
    })
    ...
    for (const html of htmls) {
        ...
        it(title, async () => {
            ...
            if (html.filePath === 'iframe.html') {
                ...
                assert(
                    ...
                    outerCompatMode + ' for outer iframe.html should be CSS1Compat as it has "<!DOCTYPE html>"'
                )
                ...
                assert(
                    ...
                    innerCompatMode + ' for iframe-inner.html should be BackCompat as it lacks "<!DOCTYPE html>"'
                )
            } else {
      ...
      `)) as string
                ...
                .replace(/\n\n/g, '')
                .replace(/blob:http:\/\/localhost:\d+\/[0-9a-z\-]+/, 'blob:http://localhost:xxxx/...')
                .replace(/data:image\/png;base64,[A-Za-z0-9+/=]{500,}/g, 'data:image/png;base64,CANVAS_GENERATED_IMAGE')
    ...
    it('correctly triggers backCompat mode and rendering', async () => {
        ...
        assert(
            ...
            compatMode + ' for compat-mode.html should be BackCompat as DOCTYPE is deliberately omitted'
        )
        const renderedHeight = (await page.evaluate('document.querySelector("center").clientHeight')) as number
...
`)
        ...
        assert(
            ...
            "rebuilt compatMode should match source compatMode, but doesn't: " + rebuildCompatMode
        )
        assert(
...
`)) as any[]
    ...
    it('correctly saves blob:images offline', async () => {
        ...
        await waitForRAF(page)
        const snapshot = (await page.evaluate('JSON.stringify(snapshot, null, 2);')) as string
    ...
    it('correctly saves images in iframes offline', async () => {
        ...
        await waitForRAF(page)
        const snapshot = (await page.evaluate('JSON.stringify(window.snapshot, null, 2);')) as string
        assert(snapshot.includes('"rr_dataURL"'))
        assert(snapshot.includes('data:image/webp;base64,'))
    })
...
describe('base64 image compression tests', function (this: ISuite) {
    ...
    ])('should replace oversized SVG <image> $attr data URIs with striped placeholder', async ({ id, attr }) => {
        const page: puppeteer.Page = await browser.newPage()
        await page.goto(`${serverURL}/html/base64-images.html`, {
            waitUntil: 'load',
        })
