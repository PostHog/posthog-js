import type { idNodeMap, MaskAttributeFn, MaskInputFn, MaskInputOptions, nodeMetaMap } from './types'
...
import { countStylesheetRules, nowMs, recordStylesheetCost } from './snapshot-cost'
...
export function stringifyStylesheet(s: CSSStyleSheet): string | null {
    ...
    try {
        ...
        const stringifiedRules = Array.from(rules, (rule: CSSRule) => stringifyRule(rule, sheetHref)).join('')
    ...
    removeNodeFromMap(n: Node) {
        ...
        if (isElement(n)) {
            ...
            if (n.nodeName === 'IFRAME' && (n as HTMLIFrameElement).contentDocument) {
                this.removeNodeFromMap((n as HTMLIFrameElement).contentDocument as unknown as Node)
            }
        }
    }
...
export function maskAttributeValue({
    ...
    if (maskAllElementAttributes && !(isGenerated && RENDERING_METADATA_ATTRIBUTES.has(toLowerCase(name)))) {
...
export function recompressBase64Image(img: HTMLImageElement, dataURL: string, type?: string, quality?: number): string {
...
export function checkDataURLSize(dataURL: string, maxLength: number | undefined): string {
