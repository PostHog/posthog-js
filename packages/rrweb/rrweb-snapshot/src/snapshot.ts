    ...
    if (name === 'src' || (name === 'href' && !(tagName === 'use' && value[0] === '#'))) {
        ...
        if (tagName === 'img' && transformedValue.startsWith('data:') && element) {
            ...
            if (dataURLOptions?.maxBase64ImageLength) {
                processedDataURL = checkDataURLSize(processedDataURL, dataURLOptions.maxBase64ImageLength)
            }
...
function onceIframeLoaded(iframeEl: HTMLIFrameElement, listener: () => unknown, iframeLoadTimeout: number) {
...
export function serializeNodeWithId(
    ...
    if (depth > maxDepth) {
        ...
        if (!_maxDepthWarned) {
            ...
            console.warn(`[rrweb] Max DOM depth (${maxDepth}) reached, skipping deeper nodes`)
            onMaxDepthReached?.()
        }
        return null
    }
