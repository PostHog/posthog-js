...
function capDataURLSize(dataURL: string, dataURLOptions?: DataURLOptions): string {
...
export function classMatchesRegex(node: Node | null, regex: RegExp, checkAncestors: boolean): boolean {
...
function onceIframeLoaded(iframeEl: HTMLIFrameElement, listener: () => unknown, iframeLoadTimeout: number): () => void {
    const noop = () => {}
...
function onceStylesheetLoaded(link: HTMLLinkElement, listener: () => unknown, styleSheetLoadTimeout: number) {
...
function serializeTextNode(
    ...
    if (isStyle && text) {
        try {
            ...
            if (n.nextSibling || n.previousSibling) {
            ...
            } else if (shouldDeferStylesheetInlining((parent as HTMLStyleElement).sheet)) {
                ...
                const stringified = stringifyStylesheet((parent as HTMLStyleElement).sheet!)
            ...
            console.warn(`Cannot get CSS styles from text's parentNode. Error: ${err as string}`, n)
        }
    ...
    if (!isStyle && !isScript && text && needsMask) {
        text = maskTextFn ? maskTextFn(text, dom.parentElement(n)) : text.replace(/[\S]/g, '*')
    }
...
function serializeElementNode(
    ...
    if (tagName === 'link' && inlineStylesheet && !needBlock) {
        ...
        let stylesheet: CSSStyleSheet | null | undefined = (n as HTMLLinkElement).sheet
        if (!stylesheet) {
    ...
    if (
        ...
        const cssText = stringifyStylesheet((n as HTMLStyleElement).sheet as CSSStyleSheet)
        if (cssText) {
            attributes._cssText = cssText
        }
    }
    ...
    if (tagName === 'dialog' && (n as HTMLDialogElement).open) {
        ...
        try {
            ;(attributes as DialogAttributes).rr_open_mode = n.matches('dialog:modal') ? 'modal' : 'non-modal'
        } catch {
    ...
    if (tagName === 'img' && inlineImages) {
        ...
        const recordInlineImage = () => {
            ...
            if (image.crossOrigin === 'anonymous') {
                if (priorCrossOrigin) {
                    attributes.crossOrigin = maskLateAttribute('crossOrigin', priorCrossOrigin)
                } else {
    ...
    if (slimDOMOptions.comment && sn.type === NodeType.Comment) {
        ...
        if (
            ...
            ((sn.tagName === 'link' && sn.attributes.rel === 'shortcut icon') ||
                (sn.tagName === 'meta' &&
                    (lowerIfExists(sn.attributes.name).match(/^msapplication-tile(image|color)$/) ||
                        ...
                        ['icon', 'apple-touch-icon', 'shortcut icon'].includes(lowerIfExists(sn.attributes.rel)))))
        ) {
            return true
            ...
            if (
                ...
                ['robots', 'googlebot', 'bingbot'].includes(lowerIfExists(sn.attributes.name))
            ) {
                return true
...
export function serializeNodeWithId(
    ...
    options: {
        ...
        onIframeListenerRegistered?: (iframeNode: HTMLIFrameElement, disposer: () => void) => void
            ...
            () => {
                ...
                if (iframeDoc && onIframeLoad) {
                    ...
                    if (serializedIframeNode) {
                        onIframeLoad(n as HTMLIFrameElement, serializedIframeNode as serializedElementNodeWithId)
                    }
                }
            },
            iframeLoadTimeout
            ...
            () => {
                if (onStylesheetLoad) {
                    ...
                    if (serializedLinkNode) {
                        onStylesheetLoad(n as HTMLLinkElement, serializedLinkNode as serializedElementNodeWithId)
                    }
                }
            },
            stylesheetLoadTimeout
...
export function slimDOMDefaults(slimDOM: 'all' | boolean | SlimDOMOptions): SlimDOMOptions {
