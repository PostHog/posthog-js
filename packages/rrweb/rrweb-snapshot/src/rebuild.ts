...
import { type serializedNodeWithId, NodeType, type elementNode, type legacyAttributes } from '@posthog/rrweb-types'
...
export function adaptCssForReplay(cssText: string, cache: BuildCache): string {
    ...
    let result = cssText
    try {
        const ast: { css: string } = postcss([mediaSelectorPlugin, pseudoClassPlugin]).process(cssText, {
            parser: safeParser,
        })
        result = ast.css
    } catch (error) {
    ...
    switch (n.type) {
        ...
        case NodeType.Element: {
                ...
                if (
                    ...
                    try {
                        ...
                        if (!warnedCustomElementNames.has(n.tagName)) {
                            ...
                            console.warn('rrweb: failed to register custom element', n.tagName, error)
                        }
                    }
                }
            ...
            for (const name in n.attributes) {
                ...
                if (tagName === 'option' && name === 'selected' && (value as legacyAttributes[typeof name]) === false) {
            ...
            const wasInFlow =
                inFlow && (specialAttributes.rr_transform === undefined || specialAttributes.rr_transform === 'none')
            ...
            if (wasInFlow && typeof specialAttributes.rr_display === 'string') {
                ...
                ;(node as HTMLElement).style.setProperty('display', display === 'inline' ? 'inline-block' : display)
            }
            ...
            for (const name in specialAttributes) {
                ...
                if (tagName === 'canvas' && name === 'rr_dataURL') {
                    ...
                    if (!image.currentSrc.startsWith('data:')) {
                        ...
                        image.setAttribute('rrweb-original-src', n.attributes.src as string)
                        image.src = value.toString()
                    }
                }
                ...
                if (name === 'rr_width') {
                    ...
                    ;(node as HTMLDialogElement).setAttribute('rr_open_mode', value as string) // keep this attribute for rrweb to trigger showModal
                }
            }
...
export function buildNodeWithSN(
    ...
    if ((n.type === NodeType.Document || n.type === NodeType.Element) && !skipChild) {
        for (const childN of n.childNodes) {
            ...
            } else if (n.type === NodeType.Document && childN.type === NodeType.DocumentType) {
