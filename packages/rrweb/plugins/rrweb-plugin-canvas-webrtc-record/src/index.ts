...
import type { RecordPlugin, ICrossOriginIframeMirror, IMirror } from '@posthog/rrweb-types'
...
export class RRWebPluginCanvasWebRTCRecord {
    ...
    constructor({
        ...
        window.addEventListener('message', (event: MessageEvent) => this.windowPostMessageHandler(event))
    ...
    public signalReceiveFromCrossOriginIframe(signal: RTCSessionDescriptionInit, source: WindowProxy) {
    ...
    private isCrossOriginIframeMessageEventContent(
        ...
        return Boolean(
            ...
            'type' in event.data &&
            'data' in event.data &&
            (event.data as CrossOriginIframeMessageEventContent).type === 'rrweb-canvas-webrtc' &&
     ...
     */
    private windowPostMessageHandler(event: MessageEvent<CrossOriginIframeMessageEventContent> | MessageEvent) {
