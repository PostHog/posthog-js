...
export class RRWebPluginCanvasWebRTCReplay {
    private canvasFoundCallback: (node: Node | RRNode, context: { id: number; replayer: Replayer }) => void
    ...
    public initPlugin(): ReplayPlugin {
        return {
            onBuild: (node: Node | RRNode, context: { id: number; replayer: Replayer }) => {
    ...
    private startStream(target: HTMLCanvasElement | HTMLVideoElement, stream: MediaStream) {
        ...
        if ('MediaStreamTrackProcessor' in window) {
            ...
            if (!ctx) throw new Error(`startStream: Could not get 2d canvas context for ${canvas.outerHTML}`)
    ...
    private flushStreams() {
        this.streams.forEach((stream) => {
            ...
            if (!nodeId) return
            const target = this.mirror?.getNode(nodeId) as HTMLCanvasElement | HTMLVideoElement | null
