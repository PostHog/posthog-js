import { type LogLevel, type LogData, PLUGIN_NAME } from '@posthog/rrweb-plugin-console-record'
        ...
        for (const level of this.config.level!) {
            if (level === 'trace') {
                replayLogger[level] = (data: LogData) => {
                    const logger = (console.log as unknown as PatchedConsoleLog)[ORIGINAL_ATTRIBUTE_NAME]
                        ? (console.log as unknown as PatchedConsoleLog)[ORIGINAL_ATTRIBUTE_NAME]
                        : console.log
                    logger(...data.payload.map((s) => JSON.parse(s) as object), this.formatMessage(data))
                }
            } else {
                replayLogger[level] = (data: LogData) => {
                    const logger = (console[level] as unknown as PatchedConsoleLog)[ORIGINAL_ATTRIBUTE_NAME]
                        ? (console[level] as unknown as PatchedConsoleLog)[ORIGINAL_ATTRIBUTE_NAME]
                        : console[level]
                    logger(...data.payload.map((s) => JSON.parse(s) as object), this.formatMessage(data))
                }
            }
...
export const getReplayConsolePlugin: (options?: LogReplayConfig) => ReplayPlugin = (options) => {
    const replayLogger = options?.replayLogger || new LogReplayPlugin(options).getConsoleLogger()
