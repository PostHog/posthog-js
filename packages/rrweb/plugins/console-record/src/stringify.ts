...
function isObjTooDeep(obj: Record<string, unknown>, limit: number): boolean {
    ...
    for (const key of keys) {
        if (isObject(obj[key]) && isObjTooDeep(obj[key] as Record<string, unknown>, limit - 1)) {
            return true
        }
    }
 ...
 */
export function stringify(obj: unknown, stringifyOptions?: StringifyOptions): string {
    ...
    const stack: unknown[] = []
    const keys: unknown[] = []
    return JSON.stringify(obj, function (key, value: string | bigint | object | null | undefined) {
        /**
        ...
        if (value instanceof Event) {
            ...
            for (const eventKey in value) {
                const eventValue = (value as unknown as Record<string, unknown>)[eventKey]
                if (Array.isArray(eventValue)) {
                    eventResult[eventKey] = pathToSelector((eventValue.length ? eventValue[0] : null) as HTMLElement)
                } else {
    ...
    function shouldIgnore(_obj: object): boolean {
         ...
         */
        if (isObject(_obj) && isObjTooDeep(_obj as Record<string, unknown>, options.depthOfLimit)) {
            return true
        }
