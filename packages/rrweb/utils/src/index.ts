...
type TypeofPrototypeOwner = typeof Node | typeof ShadowRoot | typeof MutationObserver | typeof Element
...
type WindowWithUnpatchedSymbols = typeof globalThis & Record<string, TypeofPrototypeOwner>
...
function angularZoneUnpatchedAlternative(key: keyof BasePrototypeCache) {
    const angularUnpatchedVersionSymbol = (globalThis as WindowWithZone)?.Zone?.__symbol__?.(key)
    if (angularUnpatchedVersionSymbol && (globalThis as WindowWithUnpatchedSymbols)[angularUnpatchedVersionSymbol]) {
        return (globalThis as WindowWithUnpatchedSymbols)[angularUnpatchedVersionSymbol]
    } else {
        return undefined
    }
}

export function getUntaintedPrototype<T extends keyof BasePrototypeCache>(key: T): BasePrototypeCache[T] {
    ...
    const isUntaintedAccessors = Boolean(
        ...
        accessorNames.every((accessor: keyof typeof defaultPrototype) =>
            Boolean(
                Object.getOwnPropertyDescriptor(defaultPrototype, accessor)?.get?.toString().includes('[native code]')
            )
        )
    )
...
const untaintedAccessorCache: Record<string, (this: PrototypeOwner, ...args: unknown[]) => unknown> = {}

export function getUntaintedAccessor<K extends keyof BasePrototypeCache, T extends keyof BasePrototypeCache[K]>(
    key: K,
...
type BaseMethod<K extends keyof BasePrototypeCache> = (this: BasePrototypeCache[K], ...args: unknown[]) => unknown

// eslint-disable-next-line @typescript-eslint/no-explicit-any
const untaintedMethodCache: Record<string, BaseMethod<any>> = {}
export function getUntaintedMethod<K extends keyof BasePrototypeCache, T extends keyof BasePrototypeCache[K]>(
    key: K,
    instance: BasePrototypeCache[K],
    method: T
): BasePrototypeCache[K][T] {
    const cacheKey = `${key}.${String(method)}`
    if (untaintedMethodCache[cacheKey]) return untaintedMethodCache[cacheKey].bind(instance) as BasePrototypeCache[K][T]
