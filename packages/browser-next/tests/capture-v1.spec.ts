...
describe('Capture Analytics V1', () => {
    ...
    describe('batch partitioning', () => {
        ...
        it('returns only never-attempted batches when delivery is cancelled between requests', async () => {
            ...
            const fetch = vi.fn<Parameters<BrowserFetch>, ReturnType<BrowserFetch>>(async (_input, init = {}) => {
    ...
    it('aborts a request that stalls before response headers', async () => {
        ...
        try {
            ...
            const fetch = vi.fn<Parameters<BrowserFetch>, ReturnType<BrowserFetch>>().mockImplementation(
                async (_input, init = {}) =>
    ...
    it('clamps an attempt timeout to the remaining elapsed budget', async () => {
        ...
        try {
            ...
            expect(result.error).toBeInstanceOf(Error)
            expect((result.error as Error).message).toMatch(/after 49\d(?:\.\d+)?ms$/)
            expect(vi.getTimerCount()).toBe(0)
        } finally {
            vi.useRealTimers()
        }
