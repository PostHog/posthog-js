...
Automatic analytics construction gets one additional attempt if its initial load fails. If both attempts fail, client creation still resolves with buffer-only analytics. Delivery-loading failures can be retried by explicit operations such as `flush()` and `captureImmediate()`; ordinary captures do not repeatedly retry a failed load. Concurrent loads share one attempt, and successful results are retained for that loader's lifetime.

Retries invoke the same literal dynamic import. Whether that starts another network request depends on the application bundler and browser: some native-ESM runtimes cache failed imports, including failed dependencies. Retrying cannot guarantee recovery from cached failures or deleted deployment assets. A client whose initialization attempts both failed must be recreated to attempt automatic analytics construction again.
