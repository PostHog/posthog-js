...
The root's `getFeatureFlag`, `onFeatureFlags`, and `updateFlags` are narrow optional capability delegates. Types are imported only as types; result conversion, subscriptions, bootstrap, evaluation, and persistence stay in the optional module. Disabled/unavailable reads return undefined, subscription returns a harmless disposable, and updates do nothing. Callbacks enumerate results without emitting flag-called events. Identity/group/reset notifications follow successful local state mutations, independent of capture admission, and delegate failures are contained.

Flags receives a package-private initialization capability containing the already-selected effective storage, a product-specific key, and native-observation selection. The shared `Client` contract is unchanged. Flags owns the separate `<effective core persistence key>_flags` record containing `{ distinctId, values }`; unrelated core/session/extension writes cannot replace that record. Matching identities reconcile latest persisted values before flag writes, using the shared extension's complete/partial snapshot ownership markers and per-property deltas. Native storage events or a custom adapter subscription refresh callbacks and dynamic event properties; disposal removes observation. Different identities do not adopt or overwrite one another's values without an explicit local identity transition. Reset clears the product record for the new local identity. Storage failures retain a memory-only snapshot for that instance. This is a new browser-next product layout, not a migration of legacy storage. Simultaneous read/modify/write operations remain last-writer-wins rather than an atomic transaction.
