...
Analytics owns one `EventBuffer` throughout its lifetime; `analytics-buffer.ts` contains admission and delivery-loading policy, while `analytics-delivery.ts` creates the deferred `Lane` driver for that existing buffer. The driver owns scheduling, finite flush barriers, transport, and retry coordination. It is ordinary implementation code within analytics, not a registered extension.

The package-private `chunk-loader.ts` primitive coordinates deferred invocation, one in-flight attempt, successful-result caching, and failure state. Callers can permit one shared retry after a failed attempt, with eligibility checked after failure. The loader has no extension registry or buffer responsibilities; product callbacks construct their own results, and setup remains in the core lifecycle. Each loader belongs to its caller rather than a global cache. Root initialization permits one additional analytics-construction attempt. Analytics preserves operation-triggered delivery retries: captures do not keep retrying failed loads, explicit callers can join through one failed pending attempt, and shutdown joins an already-pending load without retrying its failure. Retry invokes the unchanged literal import; it does not guarantee a fresh fetch in runtimes that cache failed modules or their dependencies.
