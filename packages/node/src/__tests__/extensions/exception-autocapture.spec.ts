...
describe('exception autocapture', () => {
  ...
  it('should honour the configured burst protection bucket size', async () => {
    jest.spyOn(ErrorTracking, 'buildEventMessage').mockResolvedValue({
      event: '$exception',
      distinctId: 'distinct-id',
      properties: { $exception_list: [{ type: 'Error' }] },
    ...
    const ph = new PostHog('TEST_API_KEY', {
      ...
      fetchRetryCount: 0,
      disableCompression: true,
      exceptionBurstProtection: { bucketSize: 3 },
    })
    ...
    // captures until rate limited (a bucket of 3 lets 2 through before the limiter kicks in)
