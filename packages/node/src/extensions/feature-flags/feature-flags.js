...
class FeatureFlagsPoller {
    ...
    async getAllFlagsAndPayloads(distinctId, groups = {}, personProperties = {}, groupProperties = {}, flagKeysToExplicitlyEvaluate) {
        ...
        await Promise.all(flagsToEvaluate.map(async (flag) => {
            try {
                const { value: matchValue, payload: matchPayload } = await this.computeFlagAndPayloadLocally(flag, distinctId, groups, personProperties, groupProperties);
        ...
        if ((condition.properties || []).length > 0) {
            for (const prop of condition.properties) {
                ...
                if (propertyType === 'cohort') {
                    ...
                    this.logMsgIfDebug(() => console.warn(`[FEATURE FLAGS] Flag dependency filters are not supported in local evaluation. ` +
        ...
        for (const prop of properties) {
            try {
                ...
                if (prop.type === 'cohort') {
                    ...
                    if (debugMode) {
                        console.warn(`[FEATURE FLAGS] Flag dependency filters are not supported in local evaluation. ` +
