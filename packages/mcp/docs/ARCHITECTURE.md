...
| `enableConversationId`       | `true`                                                                                         | Inject `conversation_id` into eligible tools that don't already declare it and stamp `$mcp_conversation_id` on their events.                                                                                                                                  |
...
| `captureModel`               | `true` (object form: `{ description }`)                                                        | Capture a model id from recognized client metadata, with an injected `llm_model` argument as fallback. Self-report capture and stripping require confirmed SDK ownership; client metadata does not depend on tool ownership.                                  |
