...
4. Replace the `tools/list`, `tools/call`, `resources/list`, `resources/read`, and `initialize` handlers on the underlying `Server` instance with wrappers, and (for `McpServer`) install a `Proxy` on `_registeredTools` so any tool registered _after_ `instrument()` is also wrapped.
