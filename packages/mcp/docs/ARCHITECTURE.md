...
| `enableConversationId`       | `false` (object form: `{ description, resultText }`) | Inject `conversation_id` into eligible tools that don't already declare it and stamp `$mcp_conversation_id` on their events. Object form means enabled — but the default is off, unlike `context`, so the predicate is `isConversationIdEnabled`, never `!== false`. See ADR-0010.  |
...
| `_conversation` output-schema mirror                              | `src/extensions/conversation-state.ts`                                |
