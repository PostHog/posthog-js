...
| `ToolCategory`   | `$mcp_tool_category`      | string                                | Product category declared on the tool's `_meta.category`. Cached from `tools/list` and (for `McpServer`) seeded from `_registeredTools`; `captureToolCall` takes it as `category`. Only on `$mcp_tool_call` |
