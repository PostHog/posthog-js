...
| `captureModel`               | `false` (object form: `{ description }`) | Capture a model id from recognized client metadata, with an injected `llm_model` argument as fallback. Self-report capture and stripping require confirmed SDK ownership; client metadata does not depend on tool ownership.                                  |
