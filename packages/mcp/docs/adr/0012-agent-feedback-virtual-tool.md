...
4. **The adoption path is a handler.** By default the tool is virtual (analytics event + acknowledgement). On the `instrument()` path, `onFeedback` routes reports to a real backend and can replace the reply. On the custom-dispatcher path the host is the handler: `prepareToolCall().isFeedback` / `feedbackReport`, `captureFeedback()`, `sendFeedbackResult()`. PostHog's own MCP server can migrate its bespoke `agent-feedback` tool onto this (`collectFeedback: { toolName: 'agent-feedback', extraProperties: ... }`).
