 ...
 * posthog-node's `prepareEventMessage` overwrites the outgoing `$groups`
 * property with the top-level `groups` field, so the group must be forwarded as
 * a first-class `groups` field — not only inside `properties.$groups`.
 */
