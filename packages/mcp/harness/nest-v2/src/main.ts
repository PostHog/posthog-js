...
async function bootstrap() {
  ...
  http.get('/__events', (_req: Request, res: Response) =>
    res.json({ events, warnings, extraShapes }),
  );
  ...
  console.error(
    `server.ready http://localhost:${port}/mcp  LEVEL=${process.env.LEVEL ?? 'high'} (stateless, dual-era)`,
  );
}
