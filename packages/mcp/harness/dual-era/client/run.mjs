...
// 2026-07-28 MUST NOT: no session header may be minted on a 2026-era exchange.
// The MUST NOT is era-scoped, not SDK-scoped — a v2 server serving the 2025 era
// legitimately mints one (ADR-0009), which the longlived rows assert via
// --header token. Make the invariant structural instead of contingent on which
// rows exist: fail fast if a fixture ever expects a header on the modern era.
if (LANE === '2026' && HEADER_EXPECT !== 'none') {
  console.error(
    `fatal: --lane 2026 requires --header none (2026-07-28 forbids minting a session header), got '${HEADER_EXPECT}'`
  )
  process.exit(1)
}
...
const results = {
  calls:
    toolCalls.length === 4 && toolCalls.map((e) => p(e, '$mcp_tool_name')).join(',') === 'echo,echo,add,fail_always',
  ...
  client:
    toolCalls.length > 0 &&
    toolCalls.every(
      (e) => p(e, '$mcp_client_name') === CLIENT_INFO.name && p(e, '$mcp_client_version') === CLIENT_INFO.version
    ),
  protocol:
    toolCalls.length > 0 &&
