...
import { events, warnings, reset } from './posthog'

async function bootstrap() {
  const app = await NestFactory.create(AppModule, { logger: ['error'] })

  const http = app.getHttpAdapter().getInstance()
  http.get('/__events', (_req: Request, res: Response) => res.json({ events, warnings }))
  ...
  console.error(`server.ready http://localhost:${port}/mcp  LEVEL=${process.env.LEVEL ?? 'high'} (v1, stateless)`)
}
