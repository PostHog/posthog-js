...
function Section({
  ...
  return (
    <section className="sdk-section" style={{ '--accent': accent } as React.CSSProperties}>
...
function App() {
  ...
  const [groupProps, setGroupProps] = useState('{"industry":"Technology","size":100}')
  ...
  const [ffPersonProps, setFfPersonProps] = useState('{"email":"test@example.com"}')
  const [ffGroupProps, setFfGroupProps] = useState('{"company":{"industry":"tech"}}')
    ...
    <div className="app">
      ...
      <div className="sections">
        ...
        <Section num={1} title="Event Capture" accent="#60a5fa">
          <div className="field-grid">
            ...
            <Field label="Groups" hint="JSON" wide>
              <textarea value={captureGroups} onChange={(e) => setCaptureGroups(e.target.value)} rows={2} />
            </Field>
            ...
            <div className="checkbox-row">
              ...
              <label className="checkbox">
                <input type="checkbox" checked={captureGeoip} onChange={(e) => setCaptureGeoip(e.target.checked)} />
                Disable GeoIP
              </label>
            </div>
          </div>
        ...
        <Section num={2} title="Identify" accent="#34d399">
          <div className="field-grid">
            <Field label="Properties" hint="JSON -- sent as $set properties" wide>
              <textarea value={identifyProps} onChange={(e) => setIdentifyProps(e.target.value)} rows={3} />
            </Field>
            <div className="checkbox-row">
              <label className="checkbox">
                <input type="checkbox" checked={identifyGeoip} onChange={(e) => setIdentifyGeoip(e.target.checked)} />
                Disable GeoIP
              </label>
            </div>
          </div>
        ...
        <Section num={3} title="Group Identify" accent="#a78bfa">
          <div className="field-grid">
            ...
            <Field label="Properties" hint="JSON" wide>
              <textarea value={groupProps} onChange={(e) => setGroupProps(e.target.value)} rows={2} />
            </Field>
            ...
            <div className="checkbox-row">
              <label className="checkbox">
                <input type="checkbox" checked={groupGeoip} onChange={(e) => setGroupGeoip(e.target.checked)} />
                Disable GeoIP
              </label>
            </div>
          </div>
        ...
        <Section num={4} title="Alias" accent="#fb923c">
          <div className="field-grid">
            ...
            <div className="checkbox-row">
              <label className="checkbox">
                <input type="checkbox" checked={aliasGeoip} onChange={(e) => setAliasGeoip(e.target.checked)} />
                Disable GeoIP
              </label>
            </div>
          </div>
        ...
        <Section num={5} title="Capture Exception" accent="#f87171">
          <div className="field-grid">
            ...
            <Field label="Error type">
              <select value={errorType} onChange={(e) => setErrorType(e.target.value as 'error' | 'string' | 'object')}>
            ...
            <Field label="Additional properties" hint="JSON" wide>
              <textarea value={exceptionProps} onChange={(e) => setExceptionProps(e.target.value)} rows={2} />
            </Field>
            <Field label="Distinct ID" hint="optional override">
              <input
        ...
        <Section num={6} title="Feature Flags" accent="#22d3ee">
          <div className="field-grid">
            ...
            <Field label="Groups" hint="JSON" wide>
              <textarea value={ffGroups} onChange={(e) => setFfGroups(e.target.value)} rows={2} />
            </Field>
            <Field label="Person properties" hint="JSON" wide>
              <textarea value={ffPersonProps} onChange={(e) => setFfPersonProps(e.target.value)} rows={2} />
            </Field>
            <Field label="Group properties" hint="JSON" wide>
              <textarea value={ffGroupProps} onChange={(e) => setFfGroupProps(e.target.value)} rows={2} />
            </Field>
            <Field label="Flag keys filter" hint="comma-separated, for getAll" wide>
              <input value={ffFlagKeys} onChange={(e) => setFfFlagKeys(e.target.value)} placeholder="flag-1, flag-2" />
            </Field>
            <div className="checkbox-row">
              <label className="checkbox">
                <input type="checkbox" checked={ffSendEvents} onChange={(e) => setFfSendEvents(e.target.checked)} />
                Send flag events
              </label>
              <label className="checkbox">
                <input type="checkbox" checked={ffGeoip} onChange={(e) => setFfGeoip(e.target.checked)} />
                Disable GeoIP
              </label>
            </div>
          </div>
          <div className="actions actions--wrap">
            <button
              ...
              onClick={() => run('getFeatureFlag', () => getFeatureFlagA(ffArgs()))}
            >
              getFeatureFlag
            </button>
            <button
              {...btnProps('isFeatureEnabled')}
              onClick={() => run('isFeatureEnabled', () => isFeatureEnabledA(ffArgs()))}
            >
              isFeatureEnabled
            </button>
            <button
              ...
              onClick={() => {
                ...
                run('getFeatureFlagPayload', () => getPayloadA({ ...args, matchValue }))
              }}
            >
            ...
            <button
              ...
              onClick={() => run('getFeatureFlagResult', () => getResultA(ffArgs()))}
            >
              getFeatureFlagResult
            </button>
            <button {...btnProps('getAllFlags')} onClick={() => run('getAllFlags', () => getAllFlagsA(ffAllArgs()))}>
              getAllFlags
            </button>
            <button
              {...btnProps('getAllFlagsAndPayloads')}
              onClick={() => run('getAllFlagsAndPayloads', () => getAllPayloadsA(ffAllArgs()))}
            >
