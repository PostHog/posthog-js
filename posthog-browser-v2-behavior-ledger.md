...
| CNS-06 | CI    | Consent gates extension capture, `sendRequest()`, lane installation, remote configuration, and extension KV reads and writes.                                                                       | Present | Browser-next extension and async-setup gating tests                       |
...
| DLV-02 | CI    | A lane owns admission, size limits, queue, flush thresholds, endpoint, serializer, compression, transport, response classification, and retry policy.                                                                                                                                | Partial | Core admission bounds present; delivery policy open    |
