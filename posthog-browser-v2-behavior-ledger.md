...
| ID     | Class | Required browser-next result                                                                                                                                                                        | Current | Primary evidence or future gate                                |
| ------ | ----- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------- | -------------------------------------------------------------- |
| CNS-01 | CI    | Consent has pending, granted, and denied states. Capture policy distinguishes pending from explicit denial.                                                                                         | Partial | `packages/browser/src/consent.ts`; differential consent tests  |
| CNS-02 | CI    | Default opt-out prevents analytics persistence and transmission until opt-in.                                                                                                                       | Partial | Differential default-denial scenario; durable persistence gate |
| CNS-03 | CI    | Explicit denial is resolved before identity, session, extension KV, queue, or network state is created.                                                                                             | Missing | Initialization trace and storage fault tests                   |
...
| CNS-12 | CI    | Consent storage failures fail closed for explicit denial and do not make telemetry failures visible as host exceptions.                                                                             | Missing | Storage fault injection                                        |
...
| ID     | Class | Required browser-next result                                                                                         | Current | Primary evidence or future gate                    |
| ------ | ----- | -------------------------------------------------------------------------------------------------------------------- | ------- | -------------------------------------------------- |
| SES-01 | CI    | A session starts on the first eligible activity, not package import.                                                 | Partial | Controlled-clock lifecycle trace                   |
| SES-02 | CI    | Inactivity beyond the configured/default idle timeout rotates the session.                                           | Partial | Differential idle scenario; D7 window decision     |
| SES-03 | CI    | A session rotates at the maximum length even when activity continues.                                                | Partial | Differential maximum-length scenario; D7           |
| SES-04 | CI    | Same-origin tabs share the active session but use distinct window IDs.                                               | Partial | Multi-client session interleavings                 |
| SES-05 | CI    | A window ID survives an ordinary reload when supported and does not leak into a duplicated or independent tab.       | Missing | Real/session-storage scenarios                     |
| SES-06 | CI    | A stale tab reads fresh shared session state before activity, rotation, reset, and unload writes.                    | Missing | Cross-tab persistence tests                        |
| SES-07 | CI    | A stale tab cannot overwrite a newer sibling session or move activity time backwards.                                | Missing | Adversarial write interleavings                    |
| SES-08 | CI    | Activity persistence is throttled well below the minimum idle timeout without making cross-tab idle decisions stale. | Partial | `packages/browser/src/__tests__/sessionid.test.ts` |
| SES-09 | CI    | Session changes publish one notification with the correct reason after state is committed.                           | Partial | `onNewSession` state-machine tests                 |
| SES-10 | CI    | Reset creates a new session and window state according to D7 without reviving stale session state.                   | Partial | Differential reset scenario; reset interleavings   |
...
| SES-12 | CI    | Session storage and timer failures do not throw and do not permit stale shared-state writes.                         | Missing | Storage/timer fault injection                      |
...
| PER-02 | CI    | Reads used for identity, consent, and session decisions refresh shared state first.                                                                  | Missing | Multi-client interleavings                                     |
...
| EVT-08 | CI    | `$set`, `$set_once`, `$unset`, and `$groups` remain in V1 event `properties`.                                                                                                                            | Partial | Capture V1 transform tests                                         |
| EVT-09 | CI    | Session and window IDs are promoted to V1 root fields and are not duplicated as stale protocol properties.                                                                                               | Missing | Capture V1 transform tests                                         |
| EVT-10 | CI    | `$lib` and `$lib_version` are not event properties in V1. `PostHog-Sdk-Info` carries SDK attribution.                                                                                                    | Missing | Capture V1 harness                                                 |
| EVT-11 | CI    | Typed V1 controls are coerced to supported strict types or omitted. One malformed control cannot reject the batch.                                                                                       | Missing | Capture V1 options corpus                                          |
| EVT-12 | CI    | Per-event and per-batch byte limits are checked before transport. An oversized single event is dropped locally with name-and-byte-count diagnostics only; later lane work remains usable.                | Missing | Capture V1/live limit tests                                        |
| EVT-13 | CI    | `onEvent` observes a deeply readonly copy of the finalized admitted analytics event. Listener failure is isolated.                                                                                       | Partial | Shared host-conformance tests                                      |
...
| ID    | Class | Required browser-next result                                                                                                                                                  | Current | Primary evidence or future gate                                     |
| ----- | ----- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------- | ------------------------------------------------------------------- |
| V1-01 | DD    | The analytics lane sends `POST /i/v1/analytics/events`.                                                                                                                       | Missing | Capture V1 harness                                                  |
| V1-02 | DD    | Fetch sends Bearer authorization, JSON content type, SDK info, attempt, request ID, request timestamp, and truthful content encoding.                                         | Missing | Capture V1 harness                                                  |
| V1-03 | DD    | Browser code does not try to set the forbidden `User-Agent` header.                                                                                                           | Missing | Browser Fetch assertion                                             |
| V1-04 | DD    | The body is `{ created_at, batch }`. It contains no project token, `api_key`, or `sent_at`.                                                                                   | Missing | Capture V1 harness                                                  |
| V1-05 | DD    | Every event contains `event`, `uuid`, `distinct_id`, `timestamp`, `options`, `properties`, and optional promoted session/window IDs.                                          | Missing | Capture V1 harness                                                  |
| V1-06 | DD    | `options` is always an object and is `{}` when empty.                                                                                                                         | Missing | Capture V1 harness                                                  |
| V1-07 | DD    | A 2xx UUID-keyed results map treats `ok` and `warning` as accepted, `drop` as terminal, and `retry` as the only per-event retry instruction.                                  | Missing | Capture V1 harness                                                  |
| V1-08 | DD    | Partial retry keeps only retry-marked events and preserves event UUIDs/timestamps, request ID, and batch `created_at`. It increments attempt and refreshes request timestamp. | Missing | Capture V1 harness                                                  |
| V1-09 | DD    | Unknown result codes are terminal. The final harness decision defines behavior for a missing UUID result.                                                                     | Missing | Capture V1 harness                                                  |
| V1-10 | DD    | Transport and approved transient failures use bounded exponential backoff, jitter, and `Retry-After` as a minimum.                                                            | Missing | Capture V1 retry tests                                              |
| V1-11 | DD    | Validation, authentication, billing, payload-size, media-type, and V1 rate-limit responses are terminal unless the canonical contract changes.                                | Missing | Capture V1 status matrix                                            |
| V1-12 | SV    | Direct regional hosts and supported reverse proxies pass CORS preflight and forward all required headers unchanged.                                                           | Blocked | Real-browser/live project tests                                     |
...
| ID     | Class | Required browser-next result                                                                                                                                                                         | Current | Primary evidence or future gate                       |
| ------ | ----- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------- | ----------------------------------------------------- |
| DLV-01 | CI    | The root statically contains one analytics lane with one bounded queue.                                                                                                                              | Missing | Root attribution and queue tests                      |
| DLV-02 | CI    | A lane owns admission, size limits, queue, flush thresholds, endpoint, serializer, compression, transport, response classification, and retry policy.                                                | Missing | Lane conformance fixture                              |
...
| DLV-05 | CI    | Normal analytics flush uses Fetch and processes the complete V1 response before it removes or retries events.                                                                                        | Missing | Capture V1 sender tests                               |
| DLV-06 | CI    | Queue count, byte, and time thresholds are bounded. On queue-count overflow, discard the oldest queued event, admit the new event, and report aggregate local loss without payload data or throwing. | Missing | Queue pressure tests                                  |
| DLV-07 | CI    | Request timeout, retry count, elapsed retry time, and backoff are bounded. Disposal cannot leave retry timers active.                                                                                | Missing | Fake-clock retry tests                                |
| DLV-08 | CI    | Offline state pauses avoidable sends and reconnect triggers prompt bounded retry without a request storm.                                                                                            | Missing | Offline/online fault tests                            |
| DLV-09 | CI    | A compact client rate limiter stops runaway capture loops and makes aggregate local drops observable without recursively rate limiting its warning.                                                  | Missing | `packages/browser/src/__tests__/rate-limiter.test.ts` |
...
| DLV-11 | CI    | `pagehide` is the primary teardown signal. `unload` is a compatibility fallback. Listeners start at initialization and are removed at disposal.                                                      | Missing | Lifecycle listener tests                              |
| DLV-12 | CI    | Teardown starts no asynchronous compression or retry timers. It attempts only data that fits one aggregate keepalive budget.                                                                         | Missing | Browser teardown tests                                |
| DLV-13 | CI    | Until V1 Beacon support is verified, teardown uses immediate Fetch with `keepalive: true` and normal V1 headers.                                                                                     | Missing | Browser teardown tests                                |
...
| PRD-15 | OP    | Remote configuration starts only for installed products, has bounded request and parse behavior, publishes a stable outcome, and cannot start a product after consent revocation or disposal.                                                            | Partial | Remote-config extension and delayed-response tests               |
...
| EXT-03 | CI    | Extensions have isolated KV and logger scopes. Disposal is idempotent and runs in reverse installation order.                                      | Partial | `tests/extensions.spec.ts`                              |
| EXT-04 | CI    | The registry contains only supplied instances and loaders. It has no static product catalog.                                                       | Present | Root module attribution                                 |
| EXT-05 | CI    | A literal dynamic import creates the optional chunk. `loadExtension()` alone is not treated as a bundle boundary.                                  | Missing | Dynamic consumer fixtures                               |
