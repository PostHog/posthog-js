...
Use one small host boundary. This is illustrative, not a frozen public contract. `LaneSink`, `Lane`, `LaneDelivery`, and any coordinator are package-private implementation types; do not export them from the package root or add them to the shared `Client` contract:
...
- The private lane isolates core admission from Capture V1 delivery. One enabled initial `$pageview` uses ordinary bounded admission after configured extensions install; navigation tracking and browser-context enrichment remain optional product behavior. Optional product lanes remain unimplemented.
...
- Identity/reset policy decisions and general conflict-safe persistence remain incomplete. Core session/window behavior matches the retained `posthog-js` analytics semantics without adding replay's proactive idle timer. The public analytics lifecycle now has a finite state-machine gate covering its supported terminal and asynchronous transitions.
...
- [ ] **P0.3**: Port the Capture Analytics V1 harness plus the legacy browser consent, identity/reset, session, queue, retry, and rate-limit regression cases.
    ...
    - [x] Add browser-next invalid distinct-ID, group type/key, and event-name regression cases.
        ...
        - [x] Run the sender through the bounded analytics queue and its admission/flush lifecycle.
...
- [x] **P2.9**: Add finite, test-only state-machine coverage for `capture`, `identify`, `group`, `reset`, `flush`, `shutdown`, and `dispose`.
    - [x] Freeze production behavior at implementation commit `66aefd006` and use the measurements above as the zero-increase runtime baseline for this gate.
    - [x] Limit the matrix to supported public API and platform transitions: closing and disposed calls, mixed flush/shutdown/dispose calls, shutdown during import/request/retry/extension cleanup, final consent denial before asynchronous completion, ordinary dependency failures, and late asynchronous completion.
    - [x] Cover public mutation attempts while closing and disposed, shared lifecycle promises, one-time delivery and cleanup, late automatic-import disposal, bounded retry cancellation, and extension cleanup rejection.
    - [x] Keep production changes outside this gate. A failing state-machine case must be reported with its existing contract, user-visible consequence, proposed fix, and measured bundle delta before any runtime change is approved.
    - [x] Add no production bytes. Open reset semantics remain governed by D3 rather than this lifecycle gate.
