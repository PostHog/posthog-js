...
- The private lane isolates core admission from Capture V1 delivery. One enabled initial `$pageview` uses ordinary bounded admission after configured extensions install; navigation tracking and browser-context enrichment remain optional product behavior. Teardown, rate limiting, and optional product lanes remain unimplemented.
...
- Identity/reset policy decisions, teardown, rate-limit, and remaining lifecycle behavior remain incomplete. Core session/window behavior now matches the retained `posthog-js` analytics semantics without adding replay's proactive idle timer.
