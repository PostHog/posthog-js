...
bash compliance/node/v2/run-compliance.sh posthog-node-compliance:local \
  "$HARNESS_IMAGE" v0 /absolute/new/legacy-acceptance acceptance
bash compliance/node/v2/run-compliance.sh posthog-node-compliance:local \
  "$HARNESS_IMAGE" v1 /absolute/new/analytics-acceptance acceptance
```

The default suite is migration parity. Passing `acceptance` as the fifth argument selects the opted-in server acceptance cases. Use a new report directory for each invocation.
