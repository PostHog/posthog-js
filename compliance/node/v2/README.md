...
When updating the pins, verify anonymous image access, `linux/amd64` support, the Debian-based Node build tools, the draft2 adapter protocol, and strict `check-report` support. Validate both profiles in GitHub Actions against the released images. SDK assertion failures remain failing checks; they must be assessed before requiring the workflow for merging. Required branch checks are a separate repository-policy decision.
