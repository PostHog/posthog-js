...
_.base64Encode = function (data) {
  ...
  do {
    ...
    tph_arr[ac++] = b64.charAt(h1) + b64.charAt(h2) + b64.charAt(h3) + b64.charAt(h4);
  } while (i < data.length);
...
_.info = {
  ...
  pageviewInfo: function pageviewInfo(page) {
    return _.strip_empty_properties({
      'ph_page': page,
      'ph_referrer': document.referrer,
      'ph_browser': _.info.browser(userAgent, navigator.vendor, windowOpera),
      'ph_platform': _.info.os()
    });
  }
...
function shouldCaptureElement(el) {
  for (var curEl = el; curEl.parentNode && !isTag(curEl, 'body'); curEl = curEl.parentNode) {
    ...
    if (_utils._.includes(classes, 'mp-sensitive') || _utils._.includes(classes, 'mp-no-capture')) {
      return false;
    }
  }
...
var autocapture = {
  ...
  _captureEvent: function _captureEvent(e, instance) {
    ...
    if ((0, _autocaptureUtils.shouldCaptureDomEvent)(target, e)) {
      ...
      _utils._.each(targetElementList, function (el) {
        ...
        } // allow users to programatically prevent captureing of elements by adding class 'mp-no-capture'


        var classes = (0, _autocaptureUtils.getClassName)(el).split(' ');

        if (_utils._.includes(classes, 'mp-no-capture')) {
      ...
      instance.capture('$web_event', props);
      return true;
    }
  },
  ...
  _maybeLoadEditor: function _maybeLoadEditor(instance) {
    try {
      ...
      if (_utils._.getHashParam(window.location.search, 'state')) {
        var state = _utils._.getHashParam(window.location.search, 'state');
  ...
  _loadEditor: function _loadEditor(instance, editorParams) {
    if (!window['_mpEditorLoaded']) {
      ...
      var editorUrl = instance.get_config('app_host') + '/static/editor.js?_ts=' + new Date().getTime();
...
PostHogLib.prototype.capture = (0, _gdprUtils.addOptOutCheckPostHogLib)(function (event_name, properties, options, callback) {
  ...
  _utils.console.log('POSTHOG REQUEST:');

  _utils.console.log(truncated_data);

  this._send_request(this.get_config('api_host') + '/e/', {
    'data': encoded_data
  }, options, this._prepare_callback(callback, truncated_data)); // this._check_and_handle_triggered_notifications(data);
...
PostHogLib.prototype.capture_pageview = function (page) {
  ...
  this.capture('ph_page_view', _utils._.info.pageviewInfo(page));
};
/**
...
function init_from_snippet() {
  ...
  posthog_master = _utils.window[PRIMARY_INSTANCE_NAME]; // Initialization

  if (_utils._.isUndefined(posthog_master)) {
    // posthog wasn't initialized properly, report error and quit
    _utils.console.critical('"posthog" object not initialized. Ensure you are using the latest version of the PostHog JS Library along with the snippet we provide.');
  ...
  var snippet_version = posthog_master['__SV'] || 0;

  if (snippet_version < 1.1) {
    // posthog wasn't initialized properly, report error and quit
    _utils.console.critical('Version mismatch; please ensure you\'re using the latest version of the PostHog code snippet.');
